...
- **파서 회귀가 채워졌습니다.** `tests/test_parser_contracts.py`
  ...
  **값이 아니라 구조를 얼립니다** — 어떤 모델이 나오는지, 어떤 필드가 `None` 이
  ...
  언제든 다시 뜰 수 있지만 이 셋은 계약입니다. `h_arv_tm` 을 `h_dpt_tm` 으로 잘못
  읽는 종류는 값도 그럴듯하고 구조도 그대로라 여기서 안 잡히며, 그것은 필드 이름을
  얼리는 `tests/test_form_contracts.py` 쪽 일입니다. 픽스처 셋은 파서가 예외를
  내는 실패 응답이고, 예외 타입과 메시지 코드까지 함께 얼립니다 — 실패도 계약입니다.
  골든을 다시 뜨려면 `KORAIL_GOLDEN_UPDATE=1 pytest`.

- **비식별화가 다시 망가지는 것을 막는 가드 일곱.** 뭉개진 한글, 한국 밖의 좌표,
  날짜·시각이 아닌 날짜·시각 필드, 열차 운임일 수 없는 금액, 자릿수가 맞지 않는
  개수·순번, 백분율이 아닌 비율, 그리고 합성임이 드러나지 않는 전화번호.
  일곱 다 취향이 아니라 **실제로 새어 나온 것**에 대응합니다 — 이 목록의 Fixed 에
  적은 사고가 그것입니다. 가드가 보는 키는 64종이고, 각 가드마다 오염을 하나씩
  심어 실제로 떨어지는지 확인했습니다(열한 가지, 전부 떨어짐).
...
- **손으로 유지하는 불변식을 확인하는 검사가 생겼습니다.** `tests/test_invariants.py`
  ...
  **스위트를 되살린 것이 아닙니다** — 그때 지운 이유는 부피였고 그 판단은 그대로입니다.
  파서 회귀만 골든 픽스처가 있어야 해서 이 파일에 없고, 이 목록 맨 앞 항목이 덮습니다.
...
- **1차 비식별화가 망가진 채였고, 값 2,018개를 고쳤습니다.** 같은 모양의 합성값으로
  바꾸는 대신 자릿수와 음절을 뒤섞어서, 열네 음절(`가나다라마바사아자차카타파하`)
  로만 이루어진 한글 뭉치, 경도 `843.94`(한국은 126~130), 날짜 `42622082`,
  태그 이름까지 깨진 HTML, 스킴이 `aakff://` 인 URL 을 남겼습니다. 고친 뒤 파서
  출력의 구조 서명 7,577개 노드가 수리 전과 같음을 확인했으므로 **움직인 것은 값뿐**
  입니다.

  **없는 사실을 지어내지 않았습니다.** 역 코드에 실제 역명을 붙이려면 코드→역명
  대응표가 있어야 하는데 저장소에도 APK 리소스에도 없습니다. 추측으로 채우면 이
  저장소가 "0531 번 역은 무엇이다"라고 거짓을 주장하게 되므로, 근거가 없는 값은
  합성임이 드러나는 플레이스홀더로 두었습니다. 그래서 역 목록은 섞여 있습니다 —
  1차 비식별화가 건드리지 않고 지나간 실제 역명(`가남`, `가평`, `강경`)은 그대로고,
  뭉개진 것만 플레이스홀더입니다.

  날짜·시각은 원본이 남긴 관계를 지켰습니다. 같은 원본값은 같은 값으로 바뀌므로 한
  행의 `h_run_dt` 와 `h_dpt_dt` 는 여전히 같고, 출발이 도착보다 앞서며,
  `runningCalendar` 의 `runDt` 는 연속된 날짜이자 같은 행의 `dayDvCd` 요일과 맞습니다.

  원문이라고 적어 둔 것이 정말 원문인지도 따로 확인했습니다: `strResult` 는
  `SUCC`/`FAIL` 뿐이고, `h_msg_cd` 13종은 모두 서버 코드 형식입니다. `*Cd` 쪽
  확인은 **한글이 뭉개졌는지만** 보아서 자릿수가 섞인 코드 하나를 놓쳤습니다 —
  아래 항목에 적습니다.

  **수리는 한 번에 끝나지 않았습니다.** 오염을 키 이름의 끝자리로 찾았기 때문에
  `h_dpt_tm` 은 고쳤는데 바로 옆 `h_dpt_tm_qb` 의 `94:45` 는 남았고, `runDt1` 의
  `21145240` 도 `boarding_date_start` 의 `66715894` 도 남았습니다. 개수와 순번은
  아예 한 종류로 세지도 않아서 잔여석 305,082 석, 6억 건짜리 조회, 97만 번째
  정차역이 그대로 있었습니다. `_prc` 로 끝나는 금액 81조원도 금액 가드의 철자
  목록에 `prc` 가 없어서 지나갔습니다.

  두 번째 수리는 추측을 그만두고 픽스처의 **키 326개를 전수로 뽑아** 하나씩
  읽었습니다. 값 346개를 43종의 키에서 더 고쳤습니다. 이번에는 값 하나씩이 아니라
  **행 단위**로 고칩니다 — 앞선 시도는 잔여석 372 석을 전체 353 석짜리 열차에
  넣었습니다. 지금은 출발 순번 < 도착 순번, 잔여석 ≤ 전체 좌석, 시작 위치 비율 <
  끝 위치 비율, 쪽수 ≤ 건수가 성립합니다. `h_dpt_tm_qb` 는 `h_dpt_tm` 에서
  끌어냈습니다 — `h_trn_no_qb == h_trn_no` 가 모든 행에서 성립하는 것을 먼저
  확인했으므로 이것은 지어낸 것이 아니라 데이터를 읽은 것입니다. 그래서 고친
  `*_tm_qb` 는 24개로, 가드에 걸린 13개보다 많습니다.

  **구조 서명이 이것을 잡지 못한 이유**도 적어 둡니다. 기준선을 망가진 입력 위에서
  떴으므로, 값 오염은 원리상 거기에 걸리지 않습니다 — 고친 값이 전부 모델로 그대로
  흘러가서 구조가 움직이지 않기 때문입니다. 값 쪽은 가드가 봅니다.

  **코드와 플래그는 그대로 둡니다.** 날짜를 합성하는 것은 API 에 대해 아무것도
  주장하지 않지만, 코드 칸에 값을 넣는 것은 API 의 어휘를 지어내는 것입니다.
  그대로 둔 넷은 `acepCustMgFlg`(`j`), `car_tp_cd`(`50564108`),
  `h_train_disc_origin_rt`(`Y`/`N` 인데 짝은 숫자), `eng_cd_val`(역명 옆의 다섯 자리
  숫자)이고, 넷 다 분기를 몰지 않습니다. 사유는 `tests/parser_map.py` 에 적었습니다.

- **자릿수만 섞인 전화번호가 하나 남아 있었습니다.** `acepCustTeln` 이
  `010-9359-3454` 였습니다. 섞었다는 것은 원본이 아니라는 뜻이지 아무의 번호도
  아니라는 뜻이 아니므로, 합성임이 한눈에 보이는 값으로 바꾸고 가드를 붙였습니다.
...
이 판을 준비하는 동안, 아래 "Added"·"Changed"·"Fixed" 절이 기록하는 소비자 동의(consent) 체계와
...
- **동의 게이트와 dry-run 미리보기가 없어졌습니다.** `consent.py` 의
  ...
  있으면 즉시 전송되고, 남은 게이트는 `safety.py` 의 라우트 허용목록, 라우트-범주
  교차검사, 그리고 폼 모양 검사 셋입니다. `pay_with_card` 와 `pay_with_fake_card` 는 여전히 분리된
  메서드입니다.
...
`src/`·`tests/`·`scripts/` 의 파이썬 파일 **전부**가 상단에 저작권·라이선스 고지를
답니다. 저작자, 저작권, SPDX 식별자(`Apache-2.0`), 원본 URL, 그리고 §4(c)·§4(b) 가
요구하는 것이 거기 있습니다.

라이선스는 **바뀌지 않았습니다.** Apache-2.0 그대로입니다. 바뀐 것은 그 라이선스가 이미
요구하던 것이 이제 파일마다 눈에 보인다는 점뿐입니다 — §4(c)는 귀속 고지를 2차 저작물의
**소스 형태에 유지**하라고 요구하는데, 유지할 고지가 파일에 없으면 그 조항은 실질적으로
`LICENSE` 파일 하나에만 걸립니다. 헤더는 각 파일이 스스로 출처를 말하게 합니다.

`NOTICE` 와 README 에 **상업적 이용을 권하지 않는다는 문단**을 더했습니다. 이것은 부탁이지
라이선스 조항이 아닙니다 — Apache-2.0 은 상업적 이용을 허용하고, §4(d) 가 `NOTICE` 의
내용은 안내일 뿐 라이선스를 변경하지 않는다고 못 박습니다. 비영리 라이선스(CC BY-NC-SA)를
검토했다가 채택하지 않았습니다: GitHub 이 NC 변종을 라이선스로 인식하지 않고
(`choosealicense.com` 의 47개 목록에 CC 계열은 `cc-by-4.0`·`cc-by-sa-4.0`·`cc0-1.0` 셋뿐),
기업 정책 스캐너가 자동 차단하며, 다른 라이선스와 결합이 불가능해지는 반면, 실제로 이
코드를 상업화할 쪽은 라이선스를 읽지 않기 때문입니다.
...
- 응답 모델의 `raw`(와 `detail_raw`)가 비교에서 빠졌습니다. 파싱된 값이 같은 두 응답은
...
- `scripts/capture_seat_inventory_evidence.py` 가 자기 스위치
  ...
  않던 스크립트였습니다. 스위치가 없으면 지금처럼 `setup_failed` 결과만 씁니다.
- `scripts/verify_706_new_live.py` 와 `scripts/retry_unprotected_live.py` 가 실행마다
  새 합성 기기로 접속하던 것을, 다른 실서버 스크립트처럼 `KORAIL_DYNAPATH_*` 환경변수
  세 개의 실제 기기로 접속합니다. 셋이 없으면 아무것도 묻기 전에 종료 코드 2 로
  멈춥니다.
- `scripts/retry_unprotected_live.py` 가 고정 날짜 2026-09-29 대신 오늘부터 14일 뒤를
  조회합니다. 그 날짜가 지나면 검색이 비어 열차가 필요한 뒷단계가 소리 없이 건너뛰어졌을
  것입니다. 이제 검색 결과가 없으면 건너뛴 단계를 밝히고 종료 코드 1 로 끝납니다.
- `scripts/retry_delivery_roundtrip.py` 가 부모 스크립트의 opt-in 검사를 그대로 쓰고,
  기기 값을 부모처럼 `KORAIL_DYNAPATH_*` 환경변수에서 읽습니다. 전에는 검사를 따로
  베껴 두었고 실행마다 새 합성 기기로 실카드 결제를 보냈습니다. 이제 이 세 값이
  없으면 비밀번호·카드를 묻기 전에 멈춥니다.
...
- **읽기 라우트 여섯 개가 필드 검사 없이 나갔습니다.** `login.Login`,
  ...
  `custId` 는 이제 가려집니다.
- **응답 27종과 기반 클래스가 `repr()` 에 서버 문구를 실었습니다.** `h_msg_txt` 는 서버가
  호출자의 입력을 되받아 적을 수 있는 민감 키인데, `BaseKorailResponse` 의 선언이
  `repr=False` 가 아니었습니다. 37개 하위 클래스가 따로 다시 선언해 막고 있었고, 그러지
  않은 나머지는 로그와 트레이스백에 문구를 남겼습니다. 이제 기반 클래스가 가리고 다시
  선언하는 곳은 없습니다.
- **`get_seat_cars(seat_attribute_code=...)` 가 좌석속성 코드를 검사 없이 보냈습니다.** 열차
  행의 코드는 세 자리 ASCII 숫자여야 하지만, 그 코드를 대신하는 인자는 무엇이든
  `txtSeatAttCd` 로 나갔습니다. 이제 같은 검사를 거쳐 `KorailProtocolError` 입니다.
- **역발행 승차권 환불 실행이 세션 만료(`P058`)에서 세션을 남겼습니다.**
  `execute_station_ticket_refund` 는 다른 상태변경 메서드와 달리 `P058` 에서
  세션과 쿠키를 비우지 않았습니다. 이제 비우고 다시 던집니다.
- **할인카드 구매 폼이 구간의 열차번호와 추가 사용자 수를 검사하지 않았습니다.** 빈
  `train_no` 가 그대로 나갔고, 7.0.6 DTO 에는 추가 사용자 한 명의 키(`_1`)만 있는데
  두 명 이상이면 없는 `_2` 키로 나갔습니다. 이제 둘 다 `KorailProtocolError` 입니다.
- **정기권·패스 구매를 `client.v7` 로 보낼 수 있었습니다.** MUTATION_HANDOFF 는 어떤
  동의로도 `passReserve`·`passPayIssue` 를 보낼 수 없다고 했지만, 7.0.6 게이트웨이는
  메서드별 동의 하나로 이 둘과 `passOtrReserve`·`postPassOtrPayIssue` 를 보낼 수
  있었습니다. 15만~25만원 결제에 취소·환불 경로가 없고 앱 자신도 보내지 않는
  호출입니다. 이제 `call()` 이 무엇보다 먼저 이름으로 거부합니다. [1.2.0 에서
  `V7MutationConsent` 자체가 없어졌으므로, 당시 기록에 있던 "동의 객체가 네 이름을
  받지 않는다"는 층은 더 이상 존재하지 않습니다.]
...
- **`strResult` 가 없는 응답을 성공으로 넘겼습니다.** 7.0.6 `CommonOut` 은 빠진
  ...
  상속하지 않는 역 정보·MaaS 역 목록과 V7 응답 모델 7개는 제외합니다. [1.2.0 에서
  레지스트리가 2행으로 줄면서 그 예외 목록도 `VerifyOnlineRefundsOut` 하나만
  남았습니다.]
...
- **1.1.0 의 docstring 정리가 문장을 잘라 놓았습니다.** APK 인용이 붙은 줄만 남기고
  ...
  그렇습니다.
- **`pay_with_card` 가 호출자의 카드를 보내는지 확인하는 테스트가 없었습니다.** 전송
  폼을 빌더 결과와 비교하던 단언이 `set(sent) == set(expected)` 라 키만 비교했고,
  다른 카드번호를 보내도 스위트 전체가 통과했습니다. 이제 폼을 값까지 비교하고
  `pay_with_fake_card` 가 보낸 폼과도 대조하며, 카드번호·비밀번호·유효기간·생년월일과
  결제 금액을 호출자가 넘긴 값과 직접 맞춰 봅니다.
- **안전 검사 테스트 둘이 주석 한 줄에 속았습니다.** 변경 전송로가 폼 모양 검사를
  부르는지는 소스에 그 글자가 있는지로, 모듈 수준 정의가 쓰이는지는 주석까지 넣은
  글자 수로 판정했습니다. 그래서 검사 호출을 주석으로 막아도, 죽은 상수를 주석에서 한
  번 언급해도 통과했습니다. 이제 두 전송로를 실제로 돌려 검사가 전송보다 먼저
  불리는지 보고, 쓰임은 구문 트리에서 셉니다. 읽기·변경 라우트 가드가 같은 것을 같은
  말로 거절하고 같은 것을 부르는지도 고정했습니다.
- **변경 메서드가 실제로 보낸 뒤 무엇을 하는지 확인하는 테스트가 없었습니다.**
  `reserve_merge`·`reserve_transfer` 가 홀드 대신 `None` 을 돌려줘도, 변경 메서드 열
  곳 모두가 세션 만료(`P058`) 뒤 로컬 세션을 지우지 않아도 스위트가 통과했습니다.
  이제 두 예약은 빌더가 만든 폼을 그대로 보내고 파싱한 홀드를 돌려주는지, 열 곳은
  `P058` 뒤 세션과 쿠키가 비는지 확인합니다. 병합 테스트의 클라이언트는 실제 전송로를
  쓰고 있어서, dry run 이 새면 실서버로 나갈 수 있었습니다. 이제는 테스트가 실패합니다.
- **7.0.6 게이트웨이의 안전 장치가 테스트에서 한 번도 실행되지 않았습니다.** 선언된
  `@Header` 만 보내고 줄바꿈이 든 값을 거절하는 검사와, 카드를 싣는 변경에 카드 종류를
  정확히 하나 요구하는 검사를 꺼도 스위트가 통과했습니다. 이제 둘 다 실제로 돌려
  확인하고, 계약 117개를 하나씩 보내 메서드·호스트·경로·헤더·값의 위치를 대조합니다.
  [1.2.0 에서 레지스트리가 2행으로 줄어, 이 검사는 남은 두 계약만 대조합니다.]
  계약 표 한 줄이 잘못 고쳐져도 알 수 있도록, 각 줄이 Retrofit 규칙(`@Field`·`@Body` 는
  본문 있는 메서드에만)을 지키는지도 따로 봅니다. `AndroidFeatures` 의 로컬 기록 읽기·
  삭제, `clear_login_state`, 세션 객체로 온 로그인 결과의 푸시 등록도 처음으로
  실행됩니다. [1.2.0 에서 `android_features.py` 와 그 테스트가 삭제됐습니다.]
- 아직 고치지 않은 결함 넷을 테스트로 적어 두었습니다. 대기열이 키 없이 "기다려"(`201`)
  라고 답하면 통과로 취급합니다. 인코딩할 수 없는 비밀번호가 "AES 키/IV 오류"로
  보고됩니다. 좌석 조회의 `seat_attribute_code` 덮어쓰기 값은 검사 없이 전송되고,
  `get_station_info` 의 기기 인자 거절은 `KorailApiError` 밖의 `ValueError` 입니다.
  각 테스트는 오늘의 동작을 단언하고, 고치는 커밋이 함께 뒤집습니다.
- 운영 스크립트 넷을 고쳤습니다. 실카드 스크립트의 `--recover` 는 요청 간격 하한
  검사를 건너뛰어 `--min-interval 0.01` 을 받아들였고, 결제 경로는 간격을 검사하기
  전에 카드부터 읽었습니다. 복구 경로의 환불 견적은 진행 가능 여부와 서버 안내 문구를
  빼고 보여 줬습니다. `capture_live_read_surface.py` 의 동의 검사는 `python -O` 가
  지우는 `assert` 였고, `retry_unprotected_live.py` 는 로그인에 실패해도 0 으로
  끝났습니다. 라이브 스크립트 넷을 따로 검사하는 테스트도 처음으로 생겼습니다.
