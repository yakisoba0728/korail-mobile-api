...
- **비식별화가 다시 망가지는 것을 막는 가드 셋.** 뭉개진 한글, 한국 밖의 좌표,
  날짜·시각이 아닌 날짜·시각 필드. 셋 다 취향이 아니라 **실제로 새어 나온 것**에
  대응합니다 — 이 목록의 Fixed 에 적은 사고가 그것입니다.
...
- **1차 비식별화가 망가진 채였고, 값 1,696개를 고쳤습니다.** 같은 모양의 합성값으로
  ...
  **없는 사실을 지어내지 않았습니다.** 역 코드에 실제 역명을 붙이려면 코드→역명
  ...
  원문이라고 적어 둔 것이 정말 원문인지도 따로 확인했습니다: `*Cd` 값에 뭉개진 것이
  하나도 없고, `strResult` 는 `SUCC`/`FAIL` 뿐이며, `h_msg_cd` 13종은 모두 서버 코드
  형식입니다.
