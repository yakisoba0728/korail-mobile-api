...
- ~~**Login form field order** — `idx` emitted before `custId`/`etrPath` differs from the Java `@Field` order; irrelevant for `x-www-form-urlencoded`.~~ **Fixed 2026-07-26** (`fix/apk-fidelity`): reordered to the declared `custId`, `etrPath`, `idx` (`LoginService.java:19`; call order `LoginDao.java:240`).
