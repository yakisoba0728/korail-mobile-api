...
- ~~**Login POST field order** — `idx` emitted before `custId`/`etrPath` differs from the `@Field` order; form params are order-independent.~~ **Fixed 2026-07-26** (`fix/apk-fidelity`): `session.py` now emits `custId`, `etrPath`, `idx` in the declared order (`LoginService.java:19`, call order `LoginDao.java:240`). The premise stayed true — a server parsing `x-www-form-urlencoded` does not care — but the body bytes differ from the app's whenever `custId`/`etrPath` are supplied, and matching them costs one dict reordering.
