...
- The current package boundary is 58 exact login/read routes and 75 public methods (62 login/read plus the consent-gated mutation methods `reserve`,
  `reserve_transfer`, `reserve_merge`, `reserve_with_discount_card`, `confirm_standby_hold`, `cancel_unpaid_hold`, `pay_with_fake_card`, `pay_with_card`, `refund`, `register_discount_card`, `extend_discount_card`, `recalculate_price`, and `add_to_cart`,
  which return a
...
current package boundary is 58 exact routes and 75 public methods.
...
- The current full offline release gate reports
  `2242 passed, 1 deselected`; only the explicitly opted-in live-service test
...
login/read transport boundary and 75 public methods on `KorailClient`. The
...
The current reviewed offline gate reports `2242 passed, 1 deselected`; the
