...
- The current package boundary is 58 exact login/read routes and 78 public methods (62 login/read, plus the two non-transmitting 비회원 identity helpers `begin_non_member`/`end_non_member`, plus the consent-gated mutation methods `reserve`,
  `reserve_transfer`, `reserve_merge`, `reserve_with_discount_card`, `confirm_standby_hold`, `cancel_unpaid_hold`, `pay_with_fake_card`, `pay_with_card`, `refund`, `verify_offline_refund_ticket`, `execute_offline_refund`, `register_discount_card`, `extend_discount_card`, and `recalculate_price`,
  which return a
...
current package boundary is 58 exact routes and 78 public methods.
...
- The current full offline release gate reports
  `2264 passed, 1 deselected`; only the explicitly opted-in live-service test
...
login/read transport boundary and 78 public methods on `KorailClient`. The
...
The current reviewed offline gate reports `2264 passed, 1 deselected`; the
