...
- The current package boundary is 61 exact login/read routes and 77 public methods (65 login/read plus the consent-gated mutation methods `reserve`,
...
- At this milestone the transport allowed 58 exact login/read routes and the
  ...
  DynaPath allowlist. (The count has since grown to 77; this line records the
...
current package boundary is 61 exact routes and 77 public methods.
...
and allows 61 exact read/login routes. The reservation, unpaid-cancel, payment,
...
- The current full offline release gate reports
  `2245 passed, 1 deselected`; only the explicitly opted-in live-service test
...
The current implementation evidence establishes 61 routes at the exact
...
The current reviewed offline gate reports `2245 passed, 1 deselected`; the
