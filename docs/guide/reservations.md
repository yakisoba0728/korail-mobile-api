...
일반 예약, 좌석 지정 예약, 예약대기, 환승 예약, 병합 예약을 차례로 다루고 마지막에 장바구니를 설명합니다.
...
cars = client.get_seat_cars(train, passenger_count=passengers.total)
car = next(c for c in cars.cars if c.car_no is not None and (c.remaining_seat_count or 0) >= passengers.total)
inventory = client.get_seat_inventory(train, car.car_no, passenger_count=passengers.total)
...
!!! warning "실제로 처리됩니다"
    `reserve`를 호출하면 고른 좌석을 점유하는 홀드가 실제로 만들어집니다.
    응답을 읽지 못해 예외가 나도 홀드는 만들어졌을 수 있으므로 다시 호출하기 전에 결과를 확인하세요.
...
!!! warning "실제로 처리됩니다"
    `reserve`는 예약대기를 실제로 신청하고, `confirm_standby_hold`는 그 홀드에 옵션을 실제로 저장합니다.
...
!!! warning "실제로 처리됩니다"
    `reserve_transfer`를 호출하면 두 구간의 좌석을 점유하는 홀드가 실제로 만들어집니다.
    응답을 읽지 못해 예외가 나도 서버에서는 처리됐을 수 있으므로 다시 호출하기 전에 결과를 확인하세요.
...
train = next(t for t in result.trains if t.merge_seat_application_flag in {"A", "G"})
first = client.reserve(train, passengers=passengers, job_type=KorailReservationJobType.MERGE_STANDING)
```
...
when = train.departure_date + train.departure_time
...
```python
hold = client.reserve_merge(train, merge.trains, passengers=passengers)
print(hold.pnr_no, hold.received_amount)
```

!!! warning "실제로 처리됩니다"
    `reserve_merge`를 호출하면 좌석을 점유하는 후속 홀드가 실제로 만들어집니다.
    응답을 읽지 못해 예외가 나도 서버에서는 처리됐을 수 있으므로 다시 호출하기 전에 결과를 확인하세요.
