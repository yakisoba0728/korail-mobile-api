...
Package coverage: 50 exact login/read routes and 57 public methods. Fifty-three
...
additionally requires `fake_card_only`, so only non-chargeable test cards are
sent). reserve/cancel/pay were verified live (fake card declined, no charge);
`refund` acts on a settled ticket and is exercised offline only. The read-only
