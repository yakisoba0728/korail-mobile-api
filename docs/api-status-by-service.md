...
Package coverage: 58 exact login/read routes and 73 public methods. Sixty-two
are audited login/read methods; the other eleven, `reserve`,
`reserve_transfer`, `reserve_merge`, `reserve_with_discount_card`,
`confirm_standby_hold`, `cancel_unpaid_hold`, `pay_with_fake_card`,
`pay_with_card`, `refund`, `register_discount_card` and
`extend_discount_card`, are consent-gated mutation methods. Each is
