...
Package coverage: 58 exact login/read routes and 77 public methods. Sixty-two
are audited login/read methods; the other fifteen, `reserve`,
...
`recalculate_price`, `create_trip_change_reservation`,
`roll_back_trip_change` and `change_reservation_passengers`, are
consent-gated mutation methods. Each is
