...
2. 변경합니다. 라우트·메서드·테스트 개수처럼 문서가 손으로 적는 값을 코드에서
   ...
   보십시오.
3. 풀 리퀘스트를 열기 전에 오프라인 게이트 두 개를 돌립니다.

   ```bash
   pip install -e ".[dev]"                                     # ruff + pyright
   ruff check src scripts                                      # 0 이어야 합니다
   pyright                                                     # 오류 0 이어야 합니다
   ```

   둘 다 네트워크 없이 통과해야 합니다. CI 는 여기에 `mkdocs build --strict` 와
   배포 검증(wheel 을 새 가상환경에 설치해 import 확인)을 더해 네 잡을 돌립니다.
   **테스트 스위트는 없앴습니다** — 동작을 지키는 것은 그 import 확인 하나뿐이고,
   파서나 가드가 깨져도 네 잡 모두 통과합니다.
   두 도구는 설정을 `pyproject.toml` 에서만 읽으므로
...
라우트 개수와 메서드 이름처럼 코드에서 유도되는 값을 README·SECURITY.md·`docs/`가
손으로 적어 둡니다. 이 값을 코드와 대조하는 테스트는 없습니다 — 공개 이름이나
라우트, 상태 변경 메서드를 바꿔도 아무것도 실패하지 않고, 문서가 조용히 틀린 값을
말하게 됩니다.
