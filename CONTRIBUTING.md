...
5. 풀 리퀘스트를 엽니다. **열기 전에 `.github/pull_request_template.md` 의 체크리스트에서
   자격증명·PNR·실제 서버 응답을 붙여넣지 말라는 항목을 읽어 주십시오.** PR 설명뿐 아니라
...
- **디컴파일된 소스를 붙여넣으면 안 됩니다.** `file:line` 을 가리키고 내용은 자기 말로
  설명해 주십시오. 와이어 이름(라우트 경로, `@Field`/`@Query` 키, 리터럴 상수값)만 짧은
...
디컴파일된 APK 인용(`file:line`)이나 최소한으로 위생처리한 재현을 우선해 주십시오.
