...
def parse_reservation_payment_response(
    ...
    **다만 "redaction 우회가 닫힌다"고 적었던 것은 틀렸습니다**(2026-09-23
    정정). ``h_tk_ret_pwd``·``h_take_name`` 같은 민감 필드는 타입 필드로도
    나오고 ``.raw`` 에도 **그대로 남습니다** — ``raw`` 원문 보존은 의도된
    계약이고, 타입 파서를 붙인다고 없어지지 않습니다. 가리는 일은 여전히
    로깅·직렬화 경계에서 :func:`~korail_mobile_api.redaction.redact_mapping`
    을 부르는 쪽의 몫입니다.
