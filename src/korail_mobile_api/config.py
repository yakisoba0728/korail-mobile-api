...
새로 만들어집니다. 모든 설치본이 같은 식별자를 보내는 것이 안티매크로 검사가
찾는 신호이기 때문입니다(:func:`_default_dynapath_config`).

실제 단말 값을 프로세스 사이에서 유지하려면 이 클래스를 직접 채우지 말고
:func:`~korail_mobile_api.live.build_config_from_env` 를 쓰면 됩니다.
User-Agent 와 DynaPath 토큰이 같은 단말을 주장하게 맞춰 주는 것은
그쪽뿐입니다.
"""
...
def enabled_dynapath_config() -> DynapathConfig:
    """DynaPath 를 켜고 기기 값을 이 구성만을 위해 새로 만듭니다.

    ``KorailConfig(enable_dynapath=True)`` 가 부르는 것이 이 함수입니다. 직접
    불러 :class:`KorailConfig` 의 ``dynapath`` 에 넣어도 결과는 같습니다.

    :func:`~korail_mobile_api.dynapath.build_default_token_settings` 를
    모듈 수준 싱글턴이 아니라 호출마다 새로 부르는 것은 의도입니다. 이
    설정에는 설치별 기기 식별자와 앱 시작 시각이 들어 있어서, 하나를 공유하면
    한 프로세스 안의 모든 클라이언트가 같은 기기·같은 시작 시각을 주장하게
    됩니다.
    """
...
class KorailConfig:
    ...
    ``Device``/``Version``/``Key`` 와 켜진 DynaPath 가 채워집니다. 필드를
    직접 바꾸기보다
    :func:`~korail_mobile_api.live.build_config_from_env` 로 실제 기기 값을
    주입하는 쪽이 낫습니다 — 기기 이름·화면 크기·SDK 레벨이 서로 맞아야
    의미가 있기 때문입니다.

    ``base_url`` 과 ``netfunnel_url`` 은 서로 다른 호스트이고 서로를 부를 수
    없습니다. API 는 ``smart.letskorail.com``, 대기열은 ``nf.letskorail.com``
    으로 각각 고정 검사됩니다.
    ...
    #: DynaPath 구성 전체. 기본은 **꺼짐** 입니다. 보통은 아래
    #: ``enable_dynapath`` 로 켜고, 토큰 제공자를 갈아끼우거나 허용목록을
    #: 바꿔야 할 때만 이쪽을 직접 넘기면 됩니다.
    ...
    #: NetFunnel 가상 대기열을 쓸지 여부. 기본은 거짓입니다.
    #:
    #: 거짓인 동안
    ...
    #: 에 닿을 수 없습니다.
    #:
    #: 지금까지 예약·취소·결제·환불과 읽기 전부가 대기열 토큰 없이
    #: 통과했습니다 — 서버가 우리를 계량하고 있지 않다는 뜻입니다. 그 상태에서
    #: 대기열을 켜면 게이트가 걸린 모든 호출에 왕복 한 번과 3초 타임아웃과
    #: 실패 모드가 얹힐 뿐이라, 폴링 경로는 실제 서버에 대고 돌아간 적이 없고
    #: 오프라인 테스트만 있습니다.
    #:
    #: 대기열은 서버 정책이고 앱은 그 클라이언트를 싣고 다닙니다 — 성수기
    #: 전용 조회 액션(``act_8_2``)이 따로 있는 것이 그 증거입니다. 대기열
    #: 모양의 실패가 보이면 그때 ``True`` 로 켜면 됩니다.
    netfunnel_enabled: bool = False
    #: DynaPath 안티오토메이션을 켭니다. **기본은 거짓입니다.**
    #:
    ...
    #: 막힙니다. 다른 경로는 켜든 끄든 그대로 나갑니다.
    #:
    #: ``dynapath`` 를 직접 켜서 넘겼다면 이 플래그는 아무것도 하지 않습니다.
    #:
    #: **필드 목록 맨 끝에 붙였습니다.** 이 앞의 순서는
    #: ``tests/test_public_contract.py`` 가 고정하고 있고, 중간에 끼우면 이미
    #: 위치 인자로 쓰던 호출의 뜻이 조용히 바뀝니다.
