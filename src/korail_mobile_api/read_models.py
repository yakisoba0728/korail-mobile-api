...
class TicketListTicket:
    ...
    #: 예약 행이 아니라 승차권 행에서 읽습니다. 한 계정 관측에서 131행 모두에 있었습니다.
    ...
    #: 앱 버튼 조건은 상세 플래그와 사용 여부 검사입니다(NormalTicketSectionKt.java:951, TicketHelper.java:3173-3199). 비교 리터럴은 보호돼
    #: 있습니다.
...
class CartItem:
    ...
    #: 코드(``CartInfo.java:40,41,42``, ``@SerialName`` 325/329/333행).
...
class DelayDiscountTicketListResponse(BaseKorailResponse):
    """지연할인권 목록과 페이지 정보를 담습니다. main_info는 DTO 밖 서버 추가 블록입니다(DelayDiscountViewOut.java:24,51). 날짜 입력 8종에서 12키를
    확인했지만 할인권 없는 응답뿐이었습니다."""
...
class DiscountCoupon:
    ...
    #: 코드 값의 의미는 미확인입니다(CouponOutInfo.java:63).
...
class TripMenuContent:
    ...
    #: ``passData``(``:45``, ``TrGdMenuLtOutPass.java:29-35``) — 정기권 조회에 필요한 연령·기간 선택지 묶음.
    #: ``PassConditionViewModel.java:1244`` 가 이 객체를 꺼내고, 같은 함수의 ``:1247-1248`` 이 ``null`` 이면 ``backAlert`` 로 화면을
    #: 되돌립니다.
...
class TrainScheduleItem:
    ...
    #: ``h_stnd_rsv_nm``(``TrainScheduleOutTrainInfo.java:1380``) — 상수 ``'-'`` 가 아니라 살아 있는 값입니다(: ``A2`` 일부 행이
    #: ``'역발매중'``).
    ...
    #: ``h_rsv_psb_nm``(``TrainScheduleOutTrainInfo.java:1296``) — 메뉴에 따라 **내용이 달라지는 화면 문구** 이지 가부 플래그가 아닙니다. 이
...
class PassScheduleMainInfo:
    ...
    #: 관측: 총건수>페이지 크기여도 next_page_flag=N, page_count=00000, page_no=1 이었습니다. 이 표본에서 Y 반복만으로는 후속 결과를 얻지 못합니다.
    #: page_size 를 늘리는 방법도 서버 상한·응답을 확인해야 하며 모든 조건의 완전성을 보장하지 않습니다.
...
class KorailPointSummaryResponse(BaseKorailResponse):
    ...
    #: 유형은 :attr:`disability_flag`(``h_hdcp_flg``)가 아니라 이 둘에 있습니다.
...
class CustomerTripInfo:
    ...
    #: ``@SerialName`` 이 없어 와이어 철자는 보호됨 이며 코틀린 필드명을 추정해 사용합니다.
    goods_no: str | None = None
    raw: Mapping[str, object] = field(default_factory=dict[str, object], compare=False)
...
class MaasCancelFeeResponse(BaseKorailResponse):
    """지원하지 않는 부가서비스 환불 수수료 응답 구조를 기록합니다. 앱은 "환불수수료 N원" 확인창에 보여 준 뒤 그 값으로 환불을
...
class PriceFare:
    ...
    #: 앱 표의 "운임"(TrainOpInfoScreenKt.java:2089). "21,600원" 처럼 쉼표와 원이 붙은 표시 문자열입니다.
    received_price: str | None = None
    #: 앱 표의 "합계"(TrainOpInfoScreenKt.java:2158).
    total_amount: str | None = None
    train_no: str | None = None
    raw: Mapping[str, object] = field(default_factory=dict[str, object], compare=False)
...
class ReservationSeatDetail:
    ...
    표본(BasketTicketDataKt.java:44)과 실서버 8/8행에서 확인했습니다. 재계산 입력의 기존 종류·객실·할인 코드는 이 좌석을 사용하며 다른 응답에서도 같은 필드가
    오는지는 검증 못 함입니다."""
...
class RefundTicketDetailResponse(BaseKorailResponse):
    ...
    #: ``h_compa_nm``/``h_compa_brth``(동승자 쌍, 아래)와는 별개의, 최상위 단일 필드 쌍입니다(``TicketDetailOut.java:410,418``).
    passenger_name: str | None = None
    passenger_birth_date: str | None = None
    companion_name: str | None = None
    companion_birth_date: str | None = None
    #: 상세 40응답(20승차권×2모드)에는 후보 키가 없었으며 목록 Y 6건도 같았습니다. 목록 키는 MyTicketListOutTicket.java:92,300, 상세 필드·setter는
    #: TicketDetailOut.java:65,1936입니다.
    pbp_acceptance_target_flag: str | None = None
    ...
    #: ``@SerialName`` 이 없어 와이어 철자는 보호됨, 코틀린 필드명을 추정해 사용합니다. 각 원소는 아직 행 단위로 모델링하지 않고 원본 그대로 노출합니다.
...
class DelayCertificateRow:
    ...
    delay_minutes: str
    #: runDt 는 마스크상 필수이지만 실서버 응답에는 없었습니다. 없으면 None 이며 운행일은 run_day 에 있습니다(예: 요일이 붙은 날짜).
    run_date: str | None = None
    raw: Mapping[str, object] = field(default_factory=dict[str, object], compare=False)
...
class TravelProduct:
    ...
    씁니다(TravelSearchViewModel.java:1070-1074). goods_no 는 get_product_detail 의 예약번호가 아닙니다."""
