...
"""기본 조회 폼 빌더. dict·순서 있는 필드 목록만 만들며 전송하지 않습니다. 이름·구조는 DTO·호출부·라이브 기록에 근거합니다. @FieldMap 선언만으로 개별 키나 순서가
증명되지는 않습니다. 이를 고정하던 테스트는 삭제되어 지금은 각 빌더의 코드가 계약입니다. 나머지 조회는 read_payloads, 상태 변경은
mutation_payloads 에 있습니다. 공통 헬퍼 _device_version·_is_ascii_digits 는 _payload_helpers 에 있습니다.
"""
import time
...
def validate_seat_inventory_inputs(
    ...
    *,
    ...
    """TrainSummary 타입, 인원 1~9, 지정 호차의 양의 정수 여부를 검사합니다. 행의 열차·역·날짜 식별자는 재검사하지 않으므로 임의 생성한 행의 유효성은 보장하지
    않습니다.
    """
...
def _validated_room_class_code(value: str) -> str:
    ...
    # 복사합니다(TrainSeatMapViewModel.java:2542,2546). PsrmType.java:19-22 의 GENERAL/SPECIAL 값은 보호됨. 이 코드의
    # 1/2 배정은 라이브 기록에 의존합니다.
    if value not in {"1", "2"}:
        raise KorailProtocolError(
...
def build_seat_car_form(
    ...
    *,
    ...
    평탄화합니다(NetworkService.java:14524-14528,15335-15343). 보호된 explicitNulls 설정은 확정할 수 없으므로 빈 문자열 처리와
    null 처리를 혼동하지 마십시오(NetworkServiceKt.java:28). menu_id 의 기본값 밖 코드는 보호돼 있으므로 맥락을 아는 호출자만 재정의하십시오.
    """
    ...
    return {
        **_device_version(config),
        ...
        # 상품이 아닌 좌석속성은 열차별 값입니다(TrainResearchIn.java:43; TrainSeatMapViewModel.java:2546,2577). 앱
        # 평탄화기는 빈 primitive 를 생략합니다 (NetworkService.java:15335-15343). 속성이 없으면 임의 기본값을 넣지 않습니다.
        **({"txtSeatAttCd": seat_attribute} if seat_attribute else {}),
        # 상품번호는 예약 입력에서 옵니다(TrainResearchIn.java:39; TrainSeatMapViewModel.java:2527). 값이 없으면 임의 기본값을
        # 넣지 않습니다.
...
def build_seat_inventory_form(
    ...
    *,
    ...
    """좌석 재고(NetworkApi.java:739-741). 호차 키는 txt 접두사 없는 srcarNo 입니다. seatAttCd·gdNo 는 없으면 생략합니다. 앱 DTO 의
    누락 기본값도 null 입니다 (TResidualSeatsResearchIn.java:107-110,127-130). ctlDvCd 는 빈 문자열이며 전송 때 제거됩니다.
    Sid 는 싣지 않습니다(TResidualSeatsResearchIn.java:65). 앱 평탄화 호출: NetworkService.java:14800-14803,15335-15343.
    explicitNulls 의 보호 리터럴 때문에 앱 null 생략까지 확정한 것은 아닙니다(NetworkServiceKt.java:28).

    isArrow 는 좌석도 기본 경로의 false 입니다. 기본 인자 마스크 0x2000(TrainSeatMapViewModel.smali:4232-4238)과 DTO 기본값
    (TResidualSeatsResearchIn.java:159-160)이 근거이며, encodeDefaults 가 보호되어 앱이 false 를 명시하는지 생략하는지는 미확정입니다.
    호차 목록 조회에서 좌석 속성을 재정의했다면 같은 seat_attribute_code 를 넘기십시오. 앱도 직전 TrainResearchIn 값을 이어
...
def build_cache_query(timestamp_ms: int | None = None) -> dict[str, str]:
    ...
    ``timestamp_ms`` 를 주지 않으면 현재 밀리초 epoch 입니다. 값은 검사하지 않고 그대로 싣습니다. 캐시를 우회하려는 값이라 서버가 내용을 보지 않습니다.
    """
...
def build_train_search_form(
    ...
    *,
    ...
    않습니다(TrainScheduleIn.java:641-650, TrainScheduleViewModel.java:3212,7340). qryDvCd 는 커서와 별개로 항상
    포함합니다.

    transfer 는 radJobId 를 바꾸며(TrainScheduleViewModel.java:3136,3212), 명시한 환승역· 후속 열차군은 목록 필드로 추가합니다.
    Sid 는 DTO 에 없습니다(TrainScheduleIn.java:95). menu_id 기본값 밖의 보호 코드는 build_seat_car_form 의 경고를 따릅니다.
    """
    ...
    # 페이징 필드 위치는 DTO 선언 순서에 따릅니다(TrainScheduleIn.java:27,33). 이 진입점은 승차권 변경용 tkPsrmClCd/tkRcvdAmt 를
    # 채우지 않습니다. 전송 생략은 @Field 바인딩이 아니라 직렬화·평탄화 과정입니다.
    ...
    if continuation is not None:
        ...
        # 있습니다(TrainScheduleOut.java:67). 앱 responseTrainSchedule 는 jadx 복원 실패로 이 자료만으로 분기 전체를 확인할 수
        # 없습니다. 기존 smali 근거 위치: TrainScheduleViewModel.smali:35654-35698,36812-36851. strJobId 비교의 보호
        # 리터럴은 미확인; 두 커서 존재 여부 조건은 라이브러리 정책입니다.
...
def build_train_schedule_special_form(
    ...
    *,
    ...
    """ScheduleViewSpecial 의 FieldMap. 기본 조회 키를 공유하며 Key 를 포함하고 Sid 는 제외합니다. 보호된 qryDvCd 값은 재구성하지 않고
    query 에서 받습니다.
    """
...
def build_ticket_list_form(
    ...
    *,
    ...
    """txtIndex 는 페이지가 아닌 목록 종류입니다(MyTicketListIn.java:62). mode 는 1/2 만 허용하며 h_page_no 는 최소 1 입니다. 앱 호출
    리터럴은 보호돼 있습니다 (MyTicketBaseViewModel.java:1029, LoginViewModel.java:1083,
    AppViewModel$executeTicketListForAutoLogin$result$1.java:67). 2026-09-22 관측: 1 은 빈 현재 목록
    WRT300005, 2 는 구매이력 128건. 날짜 범위는 그대로 전달하며 잘못된 범위는 WRT100101 을 관측했습니다. 이 표본으로 모든 계정·앱의 페이지 값을 단정하지
    않습니다.
    """
