...
이 모듈 자체는 아무것도 전송하지 않습니다. 상태변경 메서드가 요구하는 게이트
타입과, 전송 대신 돌아오는 미리보기 타입만 있습니다. I/O 없음.

기본값이 전부 막는 쪽입니다.

* 갓 만든 :class:`MutationConsent` 는 아무 범주도 허용하지 않습니다 —
  ``allow_*`` 가 전부 ``False`` 입니다.
* ``dry_run`` 이 ``True`` 라, 상태변경 호출은 폼을 만들고 검증한 뒤
  :class:`MutationPreview` 를 돌려주며 **보내지 않습니다**.
* 카드 종류 주장은 :class:`MutationConsent` 의 ``fake_card_only`` /
  ``real_card_acknowledged`` 두 플래그가 정합니다. 규칙은 그 클래스에
  적었습니다.
...
#: 코드가 아니라 이 라이브러리 자신의 게이트 어휘이므로 집합이 완전히 닫혀
#: 있습니다 — 각 값은 :class:`MutationConsent` 의 ``allow_<범주>`` 플래그 하나에
#: 대응합니다. ``"reserve"`` 예약, ``"payment"`` 결제, ``"cancel"`` 예약취소,
#: ``"refund"`` 환불, ``"discount_card"`` 할인카드 등록·해지,
#: ``"price_recalculation"`` 운임 재계산, ``"cart"`` 장바구니.
#: :data:`MUTATION_CATEGORIES` 는 같은 값들의 런타임 형태입니다.
...
class MutationConsent:
    ...
    ``allow_*`` 플래그는 범주 하나씩에 독립으로 붙고 전부 ``False`` 가
    기본입니다 — consent 는 이름을 적은 것만 허락합니다. ``dry_run`` 은
    ``True`` 가 기본이라, 상태 변경 메서드는 폼을 만들어 검증만 하고
    :class:`MutationPreview` 를 돌려주며 아무것도 전송하지 않습니다.

    ``fake_card_only``(기본 ``True``)와 ``real_card_acknowledged``(기본
    ``False``)는 어떤 종류의 카드를 보낸다고 주장하는지를 적는 자리이고, 서로
    배타적입니다. **실제로 청구되는 카드를 쓰려면 두 쪽을 다 적어야 합니다** —
    ``fake_card_only=False``(시험카드가 아니라는 주장)와
    ``real_card_acknowledged=True``(청구돼도 좋다는 주장)를 함께 켜야
    :meth:`~korail_mobile_api.client.KorailClient.pay_with_card` 가
    통과합니다. 기본값 그대로는
    :meth:`~korail_mobile_api.client.KorailClient.pay_with_fake_card` 쪽만
    열립니다. 둘 다 켠 consent 와 둘 다 끈 consent 는 모두 거절합니다 — 앞은
    모순이고 뒤는 어느 종류의 카드인지 아무 주장도 하지 않은 것이라, 어느
    쪽도 결제를 보내도 되는 상태가 아니기 때문입니다.

    카드 종류 주장이 결제를 열어 주지는 않습니다. 카드번호를 들여다보는 코드는
    없고, 실제 청구를 가르는 것은 ``allow_payment`` 와 ``dry_run`` 입니다.
    """
    ...
    #: 따로 둔 것은 이 두 경로가 상품을 사고 늘리기 때문입니다 —
    #: ``research.dcntCrdInfo.do`` 는 결제가 정산할 ``lumpStlTgtNo`` 를
    #: 돌려주고, ``reservation.dcntCrdExtn.do`` 는 카드의 유효기간을 늘립니다.
    #: 열차 예약을 허락한 사람이 할인카드 구매까지 허락한 것은 아닙니다.
    #: 이 범주는 실제 서버에 대고 시험된 적이 없습니다.
    allow_discount_card: bool = False
    #: 보류된 PNR 의 운임 재계산(``certification.PriceReCalculation``).
    #:
    #: 특히 ``allow_payment`` 와 합치지 않았습니다. 결제 동의는 **이미 제시된
    #: 특정 금액** 을 정산하라는 뜻인데, 이 경로는 정산 전에 그 금액을 서버
    #: 쪽에서 다시 씁니다. 합쳤다면 "₩X 를 내라"는 동의가 "₩X 가 얼마인지를
    #: 바꿔도 좋다"까지 뜻하게 됩니다. 예약을 만들지도 지우지도 않으므로
    #: ``allow_reserve``/``allow_cancel``/``allow_refund`` 도 아니고, 카드를
    #: 건드리지 않으므로 ``allow_discount_card`` 도 아닙니다.
    #: 이 범주는 실제 서버에 대고 시험된 적이 없습니다.
    ...
    #: 별개입니다. 이 범주는 실제 서버에 대고 시험된 적이 없습니다.
    allow_cart: bool = False
    dry_run: bool = True
    fake_card_only: bool = True
    #: 실제로 청구되는 카드번호가 평문으로 나가고 돈이 실제로 움직인다는 것을
    #: 호출자가 인정한다는 표시. 추론되지도 기본으로 켜지지도 않습니다. 규칙은
    #: 클래스 docstring 에 있습니다.
...
class MutationPreview:
    ...
    남지 않습니다. ``note`` 는 전송되지 않았다는 표시입니다.

    **마스킹되는 것은 사람이 읽는 값이지 그 값을 대신하는 코드가 아닙니다.**
    운임 재계산 미리보기에서 ``hidDcntKndCd``/``dcnt_knd_cd1`` 은 평문으로
    남고, 그 코드표에는 국가유공자(``a6/C1042B.java:140``), 장애인
    보호자(``a6/s.java:441``), 국회의원(``:442``)이 들어 있습니다 — 숫자로
    적힌 민감정보입니다. 패키지 전체가 같은 비대칭을 갖습니다
    ...
    수는 없습니다. 그래도 제3자가 읽는 곳에 남기기 전에는 미리보기 전체를
    민감정보로 다뤄야 합니다.
    """
    ...
    method: str
    route: str
    #: 값이 리스트인 것은 이상 상황이 아닙니다. ``redact_payload`` 는 같은 키가
    #: 반복되는 폼을 리스트로 보존하고, 앱도 운임 재계산에 여섯 개의
    #: ``@Field List<String>`` 를 선언합니다
...
def require_mutation_consent(
    ...
    ``category`` 는 :data:`MutationCategory` 의 일곱 값 중 하나입니다. 타입이
    ``Literal`` 이라 타입 검사기가 일곱 개를 완성해 주고 그 밖의 값을
    거부합니다. 런타임에 같은 목록이 필요하면
    :data:`MUTATION_CATEGORIES` 를 쓰면 됩니다 — 순서와 길이를 약속하는
    튜플이라 최상위로 내보내지 않습니다.

    ``consent`` 가 ``None`` 이거나 :class:`MutationConsent` 가 아니거나,
    모르는 범주이거나, 해당 ``allow_<범주>`` 가 거짓이면
    :class:`~korail_mobile_api.errors.KorailMutationNotAllowedError` 를
    올립니다. 허용되면 ``None`` 을 돌려줍니다. I/O 없음.
    """
