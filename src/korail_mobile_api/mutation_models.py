...
class KorailPassengerCounts:
    ...
    앱의 예약 요청은 섞임과 무관하게 **항상 여덟 줄** 을 보냅니다. 각 줄이
    ...
    * ``guide_dog`` 안내견 — ``"1"``, ``"173"``
    ...
    ``adult`` 만 1 이고 나머지는 0 이 기본이라, 인자 없이 만들면 어른
    1명입니다.
...
class KorailSeatAssignment:
    ...
    좌석은 좌석표 조회가 이미 돌려준 두 값으로만 식별됩니다.
    ...
    * ``seat_no`` 는
    ...
    둘 다 좌석표 조회에서 그대로 가져옵니다. 이 패키지는 좌석 식별자를 지어
    내지 않으며, "비어 있지 않은 출력 가능 ASCII" 말고는 형식을 가정하지
    않습니다. 손으로 옮겨 적지 않으려면 :meth:`from_inventory` 를 쓰면
    됩니다.
    """
    ...
    def from_inventory(
        ...
        거절합니다. 앱도 ``"Y"`` 인 좌석만 누를 수 있게 하므로
        (``com/korail/talk/ui/seat/a.java``), 사용자가 고를 수 없는 좌석을
        지정하면 서버 거절이거나 더 나쁘게는 반쪽짜리 예약이 됩니다.

        조건에 맞지 않으면 ``ValueError`` 입니다.
        """
...
class CardPayment:
    ...
    카드번호는 클라이언트 암호화 없이 평문으로 나갑니다. 어떤 종류의 카드를
    실을 수 있는지는 이 타입이 아니라 consent 가 정합니다 — 규칙은
    :class:`~korail_mobile_api.consent.MutationConsent` 에 있습니다. 기본
    consent 로는 청구되지 않는 시험카드만 보낼 수 있습니다.

    민감한 필드는 ``repr=False`` 입니다.
    """
...
class PaidTicket:
    ...
    열차번호입니다. 정산이 끝난 승차권에서 나옵니다 — 승차권 목록의 행이나
    ...
    .. warning::
       ...
       됩니다. 손으로 조립하지 말고 :meth:`from_refund_detail` 을 써야 합니다.

    가짜카드 결제는 항상 거절되므로 이것을 만들어 내지 못합니다. 실제로 발권된
    승차권은
    :meth:`~korail_mobile_api.client.KorailClient.pay_with_card` 로 명시적으로
    청구를 인정한 결제에서만 나옵니다. 환불 전송 경로 자체는 성공 응답이
    확인된 적이 없으니, 보내기 전에
    :meth:`~korail_mobile_api.client.KorailClient.get_refund_commission` 로
    환불액과 수수료를 먼저 확인해야 합니다.

    신원 필드는 전부 ``repr=False`` 입니다.
    """
    ...
    def from_refund_detail(
        ...
        *,
        ...
        :class:`PaidTicket` 을 직접 만드는 것보다 이쪽을 쓰면 됩니다.
        판매일자의 전선 키가 ``h_orgtk_sale_dt`` 로 적혀 있어
...
class DiscountCardSectionRequest:
    ...
    그대로 넘깁니다. 이 맵을 실제로 채우는 호출 지점을 앱 v6.5.0 에서 찾지
    못했고 채울 세터만 있어서, 짐작한 값을 넣지 않았습니다. 열차를 고르지 않은
    구간은 ``train_no`` 가 비어 있을 수 있습니다.
    """
...
class DiscountCardPurchaseRequest:
    ...
    인자입니다. 시계가 든 폼 빌더는 무엇을 보내는지 시험할 수 없기
    때문입니다.
    """
...
class PriceRecalculationRequest:
    ...
    담아야 합니다. ``txtPsgGridcnt`` 를 그 길이에서 유도하고 서버는 반복되는
    여섯 키를 위치로 짝짓기 때문입니다.
    """
