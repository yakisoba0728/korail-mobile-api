...
from .read_models import PassScheduleTrain
...
@dataclass(frozen=True)
class CommuterPassPurchaseRequest:
    """Everything ``pass.passReserve`` needs to reserve a 정기권.

    A 정기권 is route-bound, so the request is the product (kind / period /
    age), the day it starts, and the one or two trains the pass will be valid
    on. ``CommutationInquiryActivity.java:188-222`` (``w0``) builds it:

    * the four product scalars come straight off the schedule query the trains
      were found with (``:190-193``), and the period NAME is carried alongside
      its code -- ``CommutationBookingActivity.java:35-36`` reads both off the
      same ``PassPeriodInfo`` row, i.e. off
      :class:`~korail_mobile_api.PassPeriodOption`'s ``commuter_period_code``
      and ``display_name``.
    * ``hidUseOpenDt`` is the boarding date the schedule was queried for
      (``:194``), one of the days
      :meth:`~korail_mobile_api.KorailClient.get_pass_available_dates` returns.
    * ``trains`` is one schedule option -- the ``train_list`` of a single
      :class:`~korail_mobile_api.PassScheduleInfo`, one row for a 직통 pass and
      two for a 환승 one (``:197-221``).
    """

    pass_kind_code: str
    pass_period_code: str
    pass_period_name: str
    pass_age_code: str
    use_open_date: str
...
@dataclass(frozen=True)
class CommuterPassReservation:
    """``pass.passReserve``'s ``main_info`` — an UNPAID 정기권 reservation.

    ``CommReservationDao.CommReservationResponse.MainInfo``
    (``dao/pass/CommReservationDao.java:174-...``). The typed attributes below
    are the ones a caller acts on; :attr:`raw` is the whole object, because the
    payment that follows sends **all** of it (see
    :func:`~korail_mobile_api.mutation_payloads.build_commuter_pass_payment_form`).

    :attr:`received_amount` (``h_rcvd_amt``) is the price. It is what the app
    charges: ``CReservationConfirmActivity.java:47`` puts it in the payment
    screen's ``RECEIVED_AMOUNT`` extra with ``DISCOUNT_AMOUNT`` fixed at ``0``,
    and ``B6/AbstractC1269e.java:740`` sends
    ``hidPayAmount = t1() = s1() + getDiscountAmount()`` — which is that extra
    plus zero.
    """

    #: ``h_rcvd_amt`` — what the pass costs, and what a payment would charge.
    received_amount: str | None = None
    #: ``h_rcvd_prc`` / ``h_rcvd_fare`` — the price/fare split of that total.
    received_price: str | None = field(default=None, repr=False)
    received_fare: str | None = field(default=None, repr=False)
    #: ``h_otm_rcvd_amt`` — the one-time (single-trip) equivalent amount.
    one_time_received_amount: str | None = field(default=None, repr=False)
    #: ``h_chg_mg_no`` / ``h_chg_mg_dv_cd`` — the handle to this reservation.
    change_management_no: str | None = field(default=None, repr=False)
    change_management_division_code: str | None = field(
        default=None,
        repr=False,
    )
    #: ``h_cust_nm`` / ``h_cust_no`` — WHO the pass is for. A 정기권 is a named
    #: product; both are redacted everywhere this package prints anything.
    customer_name: str | None = field(default=None, repr=False)
    customer_no: str | None = field(default=None, repr=False)
    pass_kind_code: str | None = field(default=None, repr=False)
    pass_period_code: str | None = field(default=None, repr=False)
    pass_period_name: str | None = field(default=None, repr=False)
    pass_age_code: str | None = field(default=None, repr=False)
    #: ``h_use_open_dt`` / ``h_use_cls_dt`` — the validity window.
    use_open_date: str | None = None
    use_close_date: str | None = None
    #: ``h_use_psb_dno`` / ``h_use_psb_tno`` — usable days and trips.
    usable_day_count: str | None = field(default=None, repr=False)
    usable_trip_count: str | None = field(default=None, repr=False)
    departure_station_code: str | None = field(default=None, repr=False)
    departure_station_name: str | None = field(default=None, repr=False)
    arrival_station_code: str | None = field(default=None, repr=False)
    arrival_station_name: str | None = field(default=None, repr=False)
    transfer_station_code: str | None = field(default=None, repr=False)
    transfer_station_name: str | None = field(default=None, repr=False)
    train_group_code: str | None = field(default=None, repr=False)
    train_no_1: str | None = field(default=None, repr=False)
    train_no_2: str | None = field(default=None, repr=False)
    holiday_flag: str | None = field(default=None, repr=False)
    raw: dict[str, Any] = field(default_factory=dict, repr=False)


@dataclass(frozen=True)
class CommuterPassReservationResponse(BaseKorailResponse):
    """What ``pass.passReserve`` answers with.

    :attr:`guide` is ``h_guide``, a free-text notice the app shows in a
    confirm dialog before moving on (``CommutationInquiryActivity.java:155-166``)
    -- it is advisory, not a failure.
    """

    h_msg_txt: str | None = field(default=None, repr=False)
    guide: str | None = field(default=None, repr=False)
