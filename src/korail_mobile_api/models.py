...
"""공통 봉투와 열차·좌석 조회 모델. raw 를 가진 모델은 받은 원문을 보존합니다. frozen dataclass 도 내부 dict/list 까지 불변으로 만들지는 않습니다.
필드와 raw 는 repr·로그·직렬화에 평문 그대로 나옵니다. 승차권 등 조회는 read_models, 상태 변경은 mutation_models 에
있습니다.
"""
...
class BaseKorailResponse:
    ...
    def from_raw(cls, raw: dict[str, Any]) -> "BaseKorailResponse":
        ...
        ``raw`` 가 JSON 객체가 아니면 :class:`~korail_mobile_api.errors.KorailProtocolError` 입니다. 값이 무엇인지는 보지
        않습니다 — 실패 판정은 호출자 몫입니다.
        """
...
class AppVersionInfo:
    ...
    #: 업데이트 버튼의 CNTAURL(MobilePlusMainVersion.java:52, AppKt.java:1240,1323,1635). 2026-09-22 라이브에서
    #: 확인한 스토어 링크이며 위치 인자 호환성을 위해 뒤에 둡니다.
...
class MaasMenuItem:
    ...
    def uses_station_selection(self) -> bool:
        return (
            ...
            # ``appData`` 가 ``"N"`` 일 때만 역 목록이 비어 있습니다. 허용 목록으로 두지 않는 것은 ``{"Y", "M10", "M30"}`` 밖의 값이
            # 실제로 오기 때문입니다 — 2026-09-22 라이브: ``appData='C'``(부가서비스 코드 ``604``)가
            # ``get_maas_station_data`` 로 역 15개를 돌려줬습니다. 같은 호출에서 ``'N'``(코드 ``001``)만 0개였고,
            # ``'Y'``/``'M30'`` 은 25~111개였습니다.
...
class TrainSearchQuery:
    """열차 조회 조건. 역 이름·코드를 받으며 코드는 전송 전에 이름으로 변환합니다. 날짜·시각 형식은 YYYYMMDD·HHMMSS 입니다. 기본 열차군 109 는 라이브 기록에
    의존하며 TrainGroup.ALL 의 보호된 값은 정적으로 확인되지 않습니다(TrainGroup.java:25,29,335). include_srt 는 두 플래그를 함께
    설정하지만 앱은 SRT 또는 수서 함께 조회 조건을 사용합니다 (TrainScheduleViewModel.java:3137,3147). 보호된
    query_division_code 는 코드를 아는 경우만 지정하십시오.
    """
...
def _train_scalar(value: Any, key: str) -> str | None:
    """검색 스칼라는 문자열·JSON 정수를 허용하고 bool·float·목록·객체는 거절합니다. String
    선언(TrainScheduleOutTrainInfo.java:63,1152)보다 넓게 받는 라이브러리 정책입니다. 앱 Json 설정은 보호돼 있어 이 인용만으로 동일한 숫자
    허용을 증명하지는 못합니다 (NetworkModule.java:862, NetworkServiceKt.java:29). 이미 사라진 영 채움은 복원하지 않습니다. 후속 빌더가
    모든 자릿수를 검증한다는 보장도 없습니다.
    """
    if value is None or isinstance(value, str):
        return value
    # isinstance 가 아니라 type(...) is int 인 것은 의도다. bool 이 int 의 하위 타입이고, True 는 KORAIL 이 이런 필드로 보내는 숫자가
    # 아니다.
...
#: TrainSummary 의 ``train_no``·``goods_no``·``total_passenger_count`` 를 뺀 필드와 그 필드를 읽는 키. 세 번째 칸이 있으면
#: 첫 키가 없거나 거짓일 때 그 철자를 읽습니다(``h_trn_gp_cd`` 와 ``trnGpCd`` 등). 오류는 언제나 첫 키 이름으로 냅니다.
_TRAIN_SUMMARY_KEYS: tuple[tuple[str, str, str | None], ...] = (
    ...
    #: ``h_spe_rsv_flg`` -- ``TrainScheduleOutTrainInfo.java:108`` (Java 필드명 ``hSpRsvFlg``). 특실 예약가능
    #: 플래그.
    ...
    # 잔여 문구는 ``h_gen_rsv_nm``/``h_spe_rsv_nm`` 입니다. ``h_rsv_psb_nm``/``h_spe_rsv_psb_nm`` 는 이름과 달리 문구가
    # 아니라 **운임**입니다 -- 2026-09-22 라이브(서울→부산 20260925) 확인: 같은 행에서 ``h_gen_rsv_nm='매진'`` 인데
    ...
    #: ``TrainScheduleOutTrainInfo.java:1380``/``:1196`` 의 ``@SerialName`` 이고, 합성 생성자(``:172``)에서 이미
    #: 읽고 있던 일반실 ``h_gen_rsv_nm``(str42)·특실 ``h_spe_rsv_nm``(str47) 바로 옆의 str51/str53 입니다.
    ...
    #: ``h_stnd_rest_seat_cnt`` -- ``TrainScheduleOutTrainInfo.java:150`` (Java 필드명
    #: ``h_stnd_rest_seat_cnt`` 그대로). 입석 잔여석 수.
...
class TrainSummary:
    ...
    h_gen_rsv_nm/h_spe_rsv_nm 은 좌석 상태, h_rsv_psb_nm 은 운임일 수 있습니다. 2026-09-22 관측에서 같은 행에 매진과 47,500원이
    ...
    #: 리터럴은 보호돼 코드 배정은 정적 미확인입니다.
    merge_seat_application_flag: str | None = None
    #: 7.0.6 h_trn_sps_flg: 운휴 표시/예약 게이트용 원표 플래그.
    train_suspension_flag: str | None = None
    #: ``h_rsv_psb_nm`` — 일반실 **운임** 문구(``"47,500원"``). 이름이 예약가능("rsv_psb")처럼 보이지만 실제로 담겨 오는 값은 금액입니다.
    #: 잔여 문구는 :attr:`general_availability_name` 쪽입니다.
    general_fare_text: str | None = None
    #: ``h_spe_rsv_psb_nm`` — 특실 **운임** 문구. 위와 같은 이유로 이름과 내용이 어긋나는 키라, 잔여 문구는
    #: :attr:`special_availability_name` 에서 읽습니다.
    special_fare_text: str | None = None
    #: ``h_stnd_rsv_nm`` — 입석 잔여 화면 문구. :attr:`standing_reservation_code`(``h_stnd_rsv_cd``)가 코드이고 이쪽이
    #: 사람이 읽는 글자라, 둘 다 있어야 화면을 그대로 재현할 수 있습니다. 2026-09-22 라이브 값: ``'매진'``(서울→부산 20260925, 동대구→서울
    ...
    def from_raw(cls, raw: dict[str, Any]) -> "TrainSummary":
        ...
        주요 값은 ``h_`` 접두 철자와 접두 없는 철자를 둘 다 찾습니다 (``h_trn_no`` 와 ``trnNo`` 등). 모든 스칼라는
        ...
        return cls(
            ...
            **{
            ...
            # (TrainSeatMapViewModel.java:1974-1976,2527,2546; TrainResearchIn.java:68,275-278). 서버 행의
            # 두 후보 키가 없으면 None 입니다.
            goods_no=(
...
@dataclass(frozen=True)
class PhysicalSeat:
    """좌석표 한 자리. 좌석지정에는 표시 specification 이 아니라 식별자 seat_no 를 사용하십시오. KorailSeatAssignment.from_inventory
    로 옮길 수 있습니다. floor 는 앱 DTO 에 없습니다(TResidualSeatsResearchOutSeat.java:32-41). 그러나 이 파서는 서버 추가 문자열을
    읽으므로 항상 None 이라고 보장하지 않습니다.
    """
...
class SeatInventoryResponse(BaseKorailResponse):
    ...
    #: layout_type 은 String 선언(TResidualSeatsResearchOut.java:29)이지만 2026-09-21 서버 표본은 JSON 정수였습니다.
    #: 파서는 두 형식을 받아 str 로 정규화합니다.
    layout_type: str = ""
...
class TrainSearchMetadata:
    """행 이외의 검색 조건·커서. 2026-09-22 직통 6질의는 다음 페이지 Y 여도 필수 커서가 없었습니다. 플래그만으로 다음 요청 가능 여부를 판단하지 마십시오.
    h_menu_id 는 요청 txtMenuId 와 별도로 받은 값입니다.
    """
    ...
    #: 환승 커서 선언: TrainScheduleOut.java:28,33,67. 사용: TrainScheduleViewModel.java:7340. 응답에서 선택하는 메서드는
    #: jadx 복원 실패입니다. 기존 smali 근거는 TrainScheduleViewModel.smali:36806-36851 이며 보호된 분기 값은 미확인입니다.
    #: 라이브러리는 TransferSearchResult.next_page 에서 두 커서 존재 여부로 선택합니다.
...
class TrainSearchContinuation:
    """다음 요청에 전달하는 3튜플 커서(TrainScheduleViewModel.java:205,7340,10262-10279). 대상 필드:
    TrainScheduleIn.java:95 의 qryStNo/qryStTrnNo/qryStTrnNo2. page_count 는 이 폼에서 전송하지 않습니다.
    query_train_no2 만 빈 문자열을 허용합니다. 직접 구성하기보다 검색 결과의 next_page 를 사용하십시오.
    """
...
class TrainSearchResult:
    """직통 조회 한 페이지. 예외가 없어도 trains 가 비었는지 확인하십시오. 2026-09-22 관측: WRG000000/SUCC 에서 trn_infos 없이 빈 목록을
    반환했습니다. WRD000061 은 환승 재조회 계기이지 환승 결과 존재를 보장하지 않습니다.
    """
    ...
    def next_page(self) -> TrainSearchContinuation | None:
        """다음 페이지 Y 와 필수 커서가 모두 있을 때만 continuation 을 반환합니다. 2026-09-22 직통 6질의는 커서가 없어 None 이었습니다. 수동 커서
    ...
    def next_query_from_last_departure(
        ...
        """마지막 행의 출발 날짜·시각을 복사한 질의만 만들며 전송하지 않습니다. 행이나 날짜·시각이 없으면 None 입니다. 날짜 없이 시각만 옮기지 않습니다.
        2026-09-22 관측: 재조회 10행 중 첫 행이 앞 페이지 마지막 열차와 겹치고 9행이 새로 왔습니다. 결합 시 열차번호뿐 아니라 운행일·구간도 고려해 동일한 경계
        행을 제거하십시오. 앱 기본 화면의 동일 동작은 보장하지 않습니다. 기존 다른 화면의 근거 기록은
        TrainScheduleViewModel.smali:29590-29846,36938-36956 이며 현재 자료로 재검증하지 못했습니다.
        """
        if not self.trains:
            return None
        last = self.trains[-1]
...
class TransferItinerary:
    ...
    def transfer_station_code(self) -> str | None:
        """첫 구간 도착역과 다음 구간 출발역이 같을 때만 그 코드를 반환합니다. 다르면 None 이며 두 역을 직접 읽어야 합니다. 합치기 규칙은 라이브러리 정책입니다. 앱의
        구간별 이름 표시는 DialogsKt.java:170668,170678 참고.
        """
...
class TransferSearchResult:
    ...
    def next_page(self) -> TrainSearchContinuation | None:
        """다음 페이지 Y 이고 필수 값이 있을 때만 반환합니다. 환승 커서 둘이 모두 비어 있지 않으면 그 둘을 사용하고, 하나라도 없으면 직통 커서로 폴백합니다. 이 선택
