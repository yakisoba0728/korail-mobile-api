...
@dataclass(frozen=True)
class KorailNonMemberSession:
    """The 비회원 identity KORAIL accepts INSTEAD of a member login.

    A DIFFERENT TYPE from :class:`KorailSession`, not a flag on it. The
    distinction is structural rather than conventional: this class has no
    ``jsessionid`` field at all, because a non-member never gets one — there is
    no ``login.Login`` call, no cookie, and nothing to log out of. The server
    identifies the caller on every single request by re-sending the identity
    below, so a "non-member session" is *held credentials*, not a server-side
    session, and the two can never be passed to the same code by accident.

    This mirrors the app's own split. ``b5/h.java`` keeps ``isLogin()``
    (``:236-238``) and ``isNonMember()`` (``:244-246``) as two independent
    booleans over two independent groups of fields, cleared by two independent
    methods — ``initLoginData()`` (``:181-211``) and ``initNonmemberData()``
    (``:212-219``).

    Fields are the three the app's 비회원 등록 screen collects and stores
    (``ui/login/nonMember/NonMemberRegisterActivity.java:68-73`` →
    ``b5/h.java:344-354``):

    ``non_member_name``
        ``b5/h.java:158-160``. Goes out as ``strName`` on
        ``refunds.verifyOnlineRefunds`` (``s5/c.java:71``), as ``acepCustNm`` on
        ``refunds.executeOnlineRefunds`` (``s5/h.java:122``) and as ``hidName``
        on the non-member ticket list
        (``ui/ticket/confirm/TicketListActivity.java:945``).
    ``non_member_phone``
        ``b5/h.java:170-172``. Goes out as ``custTeln`` (``s5/h.java:123``) and
        as ``hidTeleNo`` (``TicketListActivity.java:946``).
    ``non_member_password``
        ``b5/h.java:166-168``. The 비회원 승차권 조회 password, sent as
        ``hidPwd`` (``TicketListActivity.java:947``). OPTIONAL here because the
        offline-refund pair never sends it: neither Retrofit signature declares
        a password field (``RefundService.java:17,33``).

    The app's fourth non-member field, ``nonMemberNumber``
    (``b5/h.java:161-163``), is deliberately absent: no call site in v6.5.0 was
    found that populates it, so modelling it would ship a permanently-``None``
    attribute.

    All three are PII or bearer credentials and are ``repr=False``; their
    attribute names are registered in
    :data:`~korail_mobile_api.redaction.SENSITIVE_KEYS`.
    """

    non_member_name: str = field(repr=False)
    non_member_phone: str = field(repr=False)
    non_member_password: str | None = field(default=None, repr=False)

    def __post_init__(self) -> None:
        for name in ("non_member_name", "non_member_phone"):
            value = getattr(self, name)
            if not isinstance(value, str) or not value.strip():
                raise KorailProtocolError(
                    f"KORAIL non-member session requires a non-empty {name}"
                )
        password = self.non_member_password
        if password is not None and (
            not isinstance(password, str) or not password.strip()
        ):
            raise KorailProtocolError(
                "KORAIL non-member session non_member_password must be a "
