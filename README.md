...
코레일+(구 코레일톡) 안드로이드 앱이 서버에 보내는 요청을 그대로 따라 하는 비공식 Python 클라이언트입니다.
앱 7.0.6을 뜯어 보고 경로, 폼 필드 순서, 헤더, 대기열 처리까지 앱과 똑같이 맞췄습니다.
열차 조회부터 예약, 카드 결제, 환불까지 됩니다.

KORAIL과는 관계없는 개인 프로젝트입니다. 공식 API가 아니라서 KORAIL이 서버나 앱을 바꾸면 언제든 깨질 수 있습니다.
쓰기 전에 [법적 고지](#법적-고지)는 꼭 읽어 주세요.

- **앱과 같은 요청** — 필드 순서, `User-Agent`, NetFunnel 대기열, DynaPath 토큰까지 앱이 보내는 모양 그대로입니다.
...
**SRT도 되나요?**
이 저장소는 코레일+ 앱이 쓰는 API만 다룹니다. SRT 앱용 클라이언트는 [srt-mobile-api](https://github.com/yakisoba0728/srt-mobile-api)에 있었고, 지금은 개발을 멈추고 이쪽으로 이어졌습니다.
...
- 이 프로젝트는 한국철도공사(KORAIL)나 (주)에스알(SR)과 관계없는 개인의 비공식 프로젝트이며, 어느 쪽의 후원·승인·지원도 받지 않았습니다.
...
- 권리자로서 이 저장소에 문제가 있다고 보시면 GitHub 이슈로 연락해 주세요.
