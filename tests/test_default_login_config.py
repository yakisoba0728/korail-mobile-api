...
DynaPath 는 자동화 탐지를 통과하기 위한 토큰입니다. 그것을 보낼지 말지를 이
패키지가 호출자 대신 정하지 않으므로 **기본은 꺼짐** 이고, 켜는 것은
``KorailConfig(enable_dynapath=True)`` 라고 명시적으로 말한 사람뿐입니다.

끈 채로도 대부분은 그대로 나갑니다. 토큰 없이 성공하는 것이 관측된 읽기는 그대로
가고, 관측된 거절이 있는 ``login.Login`` 만 전송 전에
``KorailDynaPathRequiredError`` 로 막힙니다. 그 자리에서 막는 이유는 서버의 거절이
사용자 문구로 "앱을 최신 버전으로 업데이트"로 위장돼 오기 때문입니다 — 그대로 두면
설정 문제가 버전 문제로 오진됩니다.

여기의 어떤 테스트도 "로그인이 성공한다"를 주장하지 않습니다. 오프라인에서 확인할
수 없는 것이고 어디에서도 주장하지 않습니다.
"""
