...
def test_status_and_progress_documents_match_current_inventory_and_coverage():
    ...
    assert "Package coverage: 58 exact login/read routes and 71 public methods" in status
    ...
    assert "71 public methods" in progress
...
def test_readme_documents_bounded_live_p0_train_reads_and_closed_requests():
    ...
    assert "58 routes and 71 public methods" in text
...
def test_docs_record_bounded_p0_live_counts_and_replay():
    ...
    assert (
        "current reviewed offline gate is `2093 passed, 1 deselected`" in readme
    )
    assert (
    ...
    assert (
        "current full offline release gate reports `2093 passed, 1 deselected`"
        in progress
    )
    assert (
    ...
    assert (
        "current reviewed offline gate reports `2093 passed, 1 deselected`; the "
