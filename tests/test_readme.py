...
def test_status_and_progress_documents_match_current_inventory_and_coverage():
    ...
    assert actual_public_methods == 75, (
...
def test_docs_document_bounded_live_p0_train_reads_and_closed_requests():
    ...
    assert "58 routes and 75 public methods" in README.read_text(encoding="utf-8")
...
def test_docs_record_bounded_p0_live_counts_and_replay():
    ...
    assert "`2242 passed, 1 deselected`" in readme
    assert (
        "current reviewed offline gate is `2242 passed, 1 deselected`" in record
    )
    assert (
    ...
    assert (
        "current full offline release gate reports `2242 passed, 1 deselected`"
        in progress
    )
    assert (
    ...
    assert (
        "current reviewed offline gate reports `2242 passed, 1 deselected`; the "
