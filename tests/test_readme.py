...
def test_status_and_progress_documents_match_current_inventory_and_coverage():
    ...
    assert "Package coverage: 50 exact login/read routes and 57 public methods" in status
...
def test_docs_describe_static_p0_menu_reads_and_exclude_crew_mutation():
    ...
    assert "50 exact read/login routes" in progress
...
def test_readme_documents_bounded_live_p0_train_reads_and_closed_requests():
    ...
    assert "50 routes and 57 public methods" in text
...
def test_docs_record_bounded_p0_live_counts_and_replay():
    ...
    assert (
        "current reviewed offline gate is `1610 passed, 1 deselected`" in readme
    )
    assert (
    ...
    assert (
        "current full offline release gate reports `1610 passed, 1 deselected`"
        in progress
    )
    assert (
    ...
    assert (
        "current reviewed offline gate reports `1610 passed, 1 deselected`; the "
