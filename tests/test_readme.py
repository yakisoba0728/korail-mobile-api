...
def test_status_and_progress_documents_match_current_inventory_and_coverage():
    ...
    assert "Package coverage: 58 exact login/read routes and 78 public methods" in status
    ...
    assert actual_public_methods == 78, (
...
def test_docs_document_bounded_live_p0_train_reads_and_closed_requests():
    ...
    assert "58 routes and 78 public methods" in README.read_text(encoding="utf-8")
    ...
    assert "58 routes and 78 public methods" in text
...
def test_docs_record_bounded_p0_live_counts_and_replay():
    ...
    assert "`2264 passed, 1 deselected`" in readme
    assert (
        "current reviewed offline gate is `2264 passed, 1 deselected`" in record
    )
    assert (
    ...
    assert (
        "current full offline release gate reports `2264 passed, 1 deselected`"
        in progress
    )
    assert (
    ...
    assert (
        "current reviewed offline gate reports `2264 passed, 1 deselected`; the "
