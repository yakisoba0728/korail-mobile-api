...
def test_status_and_progress_documents_match_current_inventory_and_coverage():
    ...
    assert "Package coverage: 61 exact login/read routes and 77 public methods" in status
    ...
    assert "61 exact login/read routes" in progress
...
def test_docs_describe_static_p0_menu_reads_and_exclude_crew_mutation():
    ...
    assert "61 exact read/login routes" in progress
...
def test_docs_document_bounded_live_p0_train_reads_and_closed_requests():
    ...
    assert "61 routes and 77 public methods" in README.read_text(encoding="utf-8")
    ...
    assert "61 routes and 77 public methods" in text
...
def test_docs_record_bounded_p0_live_counts_and_replay():
    ...
    assert "`2245 passed, 1 deselected`" in readme
    assert (
        "current reviewed offline gate is `2245 passed, 1 deselected`" in record
    )
    assert (
    ...
    assert (
        "current full offline release gate reports `2245 passed, 1 deselected`"
        in progress
    )
    assert (
    ...
    assert (
        "current reviewed offline gate reports `2245 passed, 1 deselected`; the "
