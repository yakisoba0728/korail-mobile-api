...
def bootstrap(**crypto):
    return [
        envelope(),
        envelope(**{"app.login.cphd": {"idx": "synthetic-index", "key": KEY, "pwdAESCphd": "N", **crypto}}),
    ]
...
def test_login_success_code_and_session_cookie(factory, code, result):
    ...
    client, requests = factory(
        [
            *bootstrap(),
            login_response(code, result, strMbCrdNo="synthetic-card", strCustNo="synthetic-customer"),
        ]
    )
...
@pytest.mark.parametrize(
    "code",
    [c for c in LOGIN_HASHES if c not in {"WRC000116", "WRC000420"}] + ["WRC000288", "SYNTHETIC-UNKNOWN"],
)
...
def test_pending_relogin_clears_old_cookie(factory):
    client, requests = factory(
        [*bootstrap(), login_response("WRC000116", "FAIL"), *bootstrap(), login_response()]
    )
...
def test_login_overrides_and_common_code_repeated_fields(factory):
    ...
    client.login(
        ID,
        PASSWORD,
        input_flag="synthetic-flag",
        check_valid_pw="N",
        cust_id="synthetic-cust",
        etr_path="synthetic-entry",
    )
...
def test_p058_commonout_even_with_optional_envelope(require_result, common_out, raise_on_fail, missing):
    ...
    with pytest.raises(E.KorailSessionExpiredError) as caught:
        parse_base_response(
            raw, require_result=require_result, common_out=common_out, raise_on_fail=raise_on_fail
        )
...
@pytest.mark.parametrize(
    "field, attribute", [("h_msg_cd", "h_msg_cd"), ("h_msg_txt", "h_msg_txt"), ("strResult", "str_result")]
)
...
def test_non2xx_before_json_or_app_error(factory, status, json_body):
    ...
    raw = envelope("P058", "FAIL")
    response = (
        httpx.Response(status, json=raw) if json_body else httpx.Response(status, content=b"synthetic non-json")
    )
...
def test_common_fields_headers_override_and_empty_omission(factory):
    ...
    config = KorailConfig(
        ...
        dynapath=DynapathConfig(
            ...
            header_name="X-Synthetic-Token",
            device_name="synthetic-model",
...
def test_none_provider_token_is_omitted(factory):
    config = KorailConfig(
        base_url="https://offline.invalid",
        netfunnel_enabled=False,
        dynapath=DynapathConfig(enabled=True, token_provider=lambda context: None),
    )
...
def test_default_configuration_and_explicit_overrides():
    ...
    with pytest.raises(ValueError):
        KorailConfig(
            disable_dynapath=True, dynapath=DynapathConfig(enabled=True, token_provider=lambda context: None)
        )
    with pytest.raises(ValueError):
...
@pytest.mark.parametrize(
    "code",
    ["ERR911081", "ERR800056", "S-ERR911411", "S021", "WRR664325", "WRR700001", "WRR800029", "WRX000007"],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "0123456789abcdef",
            "synthetic-password",
            "d2pDQ3FIZ3ZQdXV2d1BxUkE5azRCV1RZSWJFVk5Oc2ZrS0o5K09wOHU0RT0=\n",
        ),
