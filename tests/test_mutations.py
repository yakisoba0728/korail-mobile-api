...
from korail_mobile_api.constants import KorailReservationJobType as Job, KorailSeatClass as Cabin
...
from korail_mobile_api.mutation_models import (
    CardPayment, CartAddRequest, DiscountCardAdditionalUser, DiscountCardPurchaseRequest,
    DiscountCardSectionRequest, DiscountCardTicket, KorailPassengerCounts,
    KorailSeatAssignment, PaidTicket, PriceRecalculationRequest, PriceRecalculationRow,
    ReservationHoldResponse, ReservationJourney, StationRefundExecutionRequest,
...
BASE = {"strResult": "SUCC", "h_msg_cd": "SYNTH-SUCCESS", "h_msg_txt": "synthetic response", "extra": {"keep": [1, 2]}}
...
def train(second: bool = False, **changes: Any) -> TrainSummary:
    values = dict(
        train_no="90002" if second else "90001", train_group_code="100",
        train_class_code="0A", run_date="20991230", departure_date="20991230",
        departure_time="130000" if second else "100000", arrival_time="150000" if second else "120000",
        departure_station_code="9002" if second else "9001", arrival_station_code="9003" if second else "9002",
        departure_construction_order="003", arrival_construction_order="009",
        departure_run_order="002", arrival_run_order="007", seat_attribute_code="015",
        general_reservation_code="11", special_reservation_code="11",
        standing_reservation_code="13", merge_seat_application_flag="A", wait_reservation_flag=" 9",
    )
...
def hold(**changes: Any) -> ReservationHoldResponse:
    values = dict(str_result="SUCC", pnr_no=PNR, journey_count="0001", window_no="SYNTH-WINDOW",
                  temporary_job_sequence_1="001234", temporary_job_sequence_2="005678",
                  received_amount="1200", total_price="9900",
...
def ticket() -> PaidTicket:
    return PaidTicket(PNR, "20991230", "SYNTH-ORIGINAL-WINDOW", "SYNTH-ORIGINAL-SEQ", "SYNTH-RETURN-PWD", "90001", "SYNTH-PBP")


def merge_rows(standing: bool = True) -> tuple[TrainScheduleItem, ...]:
    return (
        TrainScheduleItem(train_no="90001", arrival_station_code="9099", arrival_construction_order="004", arrival_run_order="003",
                          general_reservation_code="13" if standing else "11", standing_reservation_code="11" if standing else "13"),
        TrainScheduleItem(train_no="90001", arrival_station_code="9002", arrival_construction_order="009", arrival_run_order="007",
                          general_reservation_code="11", standing_reservation_code="13"),
    )


def hold_raw() -> dict[str, Any]:
    return {**copy.deepcopy(BASE), "h_pnr_no": PNR, "h_jrny_cnt": "0001", "h_wct_no": "SYNTH-WINDOW",
            "h_tmp_job_sqno1": "001234", "h_tmp_job_sqno2": "005678", "h_tot_rcvd_amt": "001200", "h_tot_prc": "9900",
            "jrny_infos": {"jrny_info": [{"h_jrny_sqno": "0007", "h_rsv_chg_no": "009", "h_trn_no": "90001",
                "seat_infos": {"seat_info": [{"h_seat_no": "1A", "h_rcvd_amt": "000700"}, {"h_seat_no": "1B", "h_rcvd_amt": 500}]}}]}}


def reservation_form(*, transfer: bool = False, job: str = "1101", stnd: str = "N", cabin2: str = "1") -> dict[str, Any]:
    """TicketReservationIn.java:80; TicketReservationInJrny.java:69; TrainScheduleViewModel.java:2932,3004."""
    result = {**COMMON, "txtMenuId": "11", "txtJobId": job, "hidFreeFlg": "N", "txtStndFlg": stnd,
              "txtTotPsgCnt": "1", "txtCompaCnt1": "1", "txtPsgTpCd1": "1", "txtDiscKndCd1": "000",
              "txtSeatAttCd1": "000", "txtSeatAttCd2": "000", "txtSeatAttCd3": "000", "txtSeatAttCd4": "015",
              "txtSeatAttCd5": "000", "txtJrnyCnt": "2" if transfer else "1"}
    ...
    for i, (number, departure, arrival, time, cabin) in enumerate(rows, 1):
        row = {"txtJrnyTpCd": "14" if transfer else "11", "txtJrnySqno": f"{i:03}", "txtTrnNo": number,
               "txtTrnClsfCd": "0A", "txtTrnGpCd": "100", "txtRunDt": "20991230", "txtDptDt": "20991230", "txtDptTm": time,
               "txtDptRsStnCd": departure, "txtDptStnConsOrdr": "003", "txtDptStnRunOrdr": "002",
               "txtArvRsStnCd": arrival, "txtArvStnConsOrdr": "009", "txtArvStnRunOrdr": "007", "txtChgFlg": "N", "txtPsrmClCd": cabin}
...
def payment_form(**changes: str) -> dict[str, Any]:
    ...
    return {**COMMON, "hidPnrNo": PNR, "hidWctNo": "SYNTH-WINDOW", "hidTmpJobSqno1": "001234", "hidTmpJobSqno2": "005678",
            "hidRsvChgNo": "009", "hidInrecmnsGridcnt": "1", "hidStlMnsSqno1": "1", "hidStlMnsCd1": "02",
            "hidMnsStlAmt1": "1200", "hidCrdInpWayCd1": "@", "hidStlCrCrdNo1": "0000000000000000",
            "hidVanPwd1": "00", "hidCrdVlidTrm1": "9912", "hidIsmtMnthNum1": "0", "hidAthnDvCd1": "J",
            "hidAthnVal1": "000000", "hiduserYn": "Y", **changes}


def refund_form(**changes: str) -> dict[str, Any]:
    """MyTicketDetailViewModel.java:1521; RefundTicketIn.java:66."""
    return {**COMMON, "txtPnrNo": PNR, "h_orgtk_sale_dt": "20991230", "h_orgtk_sale_wct_no": "SYNTH-ORIGINAL-WINDOW",
            "h_orgtk_sale_sqno": "SYNTH-ORIGINAL-SEQ", "h_orgtk_ret_pwd": "SYNTH-RETURN-PWD", "h_mlg_stl": "N",
            "pbpAcepTgtFlg": "SYNTH-PBP", **changes}
...
def verification_raw() -> dict[str, Any]:
    return {**copy.deepcopy(BASE), "rcvd_amt": "1200", "ret_amt": "1100", "ret_fee": "100", "poppMsg": "synthetic popup", "strMsg": "synthetic result",
            "orgtkinfo_list": [{"pnr_no": PNR, "ogtk_sale_dt": "20991230", "ogtk_sale_wct_no": "SYNTH-ORIGINAL-WINDOW",
              "ogtk_sale_sqno": "SYNTH-ORIGINAL-SEQ", "ogtk_ret_pwd": "SYNTH-RETURN-PWD", "tk_knd_cd": "SYNTH-KIND",
              "ret_dv_cd": "SYNTH-DIV", "ret_rsn_cd": "SYNTH-REASON"}]}


def execution_request() -> StationRefundExecutionRequest:
    return StationRefundExecutionRequest(PNR, "20991230", "SYNTH-ORIGINAL-WINDOW", "SYNTH-ORIGINAL-SEQ", "SYNTH-RETURN-PWD",
                                         "SYNTH-DIV", "SYNTH-REASON", "SYNTH-KIND", "00000000000", "1100", "100", "SYNTH-CUSTOMER")
...
def execution_form() -> dict[str, Any]:
    ...
    return {**COMMON, "pnrNo": PNR, "ogtkSaleDt": "20991230", "ogtkSaleWctNo": "SYNTH-ORIGINAL-WINDOW", "ogtkSaleSqno": "SYNTH-ORIGINAL-SEQ",
            "ogtkRetPwd": "SYNTH-RETURN-PWD", "retDvCd": "SYNTH-DIV", "retRsnCd": "SYNTH-REASON", "tkKndCd": "SYNTH-KIND",
            "custTeln": "00000000000", "retAmt": "1100", "retFee": "100", "acepCustNm": "SYNTH-CUSTOMER"}


def purchase_request() -> DiscountCardPurchaseRequest:
    return DiscountCardPurchaseRequest("SYNTH-N-KIND", "SYNTH-CUSTOMER-NO", "20991230", "10", (
...
def purchase_form() -> dict[str, Any]:
    ...
    return {**COMMON, "dcntCrdKndMgNo": "SYNTH-N-KIND", "custMgNo": "SYNTH-CUSTOMER-NO", "vlidTrmStDt": "20991230", "usePsbTno": "10", "jrnyCnt": "2",
            "jrnyTpCd_1": "11", "runDt_1": "20991230", "trnNo_1": "90001", "dptRsStnCd_1": "9001", "arvRsStnCd_1": "9002",
            "jrnyTpCd_2": "14", "runDt_2": "20991231", "trnNo_2": "90002", "dptRsStnCd_2": "9002", "arvRsStnCd_2": "9003",
            "apdUsrCnt": "1", "custMgNo_1": "SYNTH-SECOND-NO", "apdCustName_1": "SYNTH-SECOND-NAME", "apdCustTeln_1": "00000000000"}


def recalc_request(**changes: Any) -> PriceRecalculationRequest:
    return PriceRecalculationRequest(PNR, (
        PriceRecalculationRow("1", "1", "000", "", "", ""),
        PriceRecalculationRow("3", "2", "SYNTH-OLD", "SYNTH-NEW", "SYNTH-CERT", "SYNTH-FAMILY")), **changes)


def recalc_form(**changes: Any) -> dict[str, Any]:
    """NetworkApi.java:583-584: six repeated unnumbered lists preserve empty positions."""
    return {**COMMON, "hidPnrNo": PNR, "txtJobId": "1101", "txtPsgGridcnt": "2",
            "psg_tp_dv_cd": ["1", "3"], "psrm_cl_cd": ["1", "2"], "dcnt_knd_cd1": ["000", "SYNTH-OLD"],
            "hidDcntKndCd": ["", "SYNTH-NEW"], "hidDscpNo": ["", "SYNTH-CERT"], "hidFmlyNo": ["", "SYNTH-FAMILY"], **changes}


def payment_raw() -> dict[str, Any]:
    return {**copy.deepcopy(BASE), "h_rsv_no": PNR, "h_tot_rcvd_amt": "1200", "h_im_flg": "SYNTH-IMAGE", "h_stl_amt": "1200",
            "tk_coupon_info": [{"h_coup_no": "SYNTH-COUPON", "extra_coupon": 1}],
            "tk_infos": {"tk_info": [{"h_tk_sqno": "01", "h_sale_dt": "20991230", "h_sale_sqno": "SYNTH-ISSUE-SEQ", "h_tk_ret_pwd": "SYNTH-PWD", "h_tot_rcvd_amt": "1200"}]},
            "stl_infos": {"stl_info": [{"h_stl_rlt": "SYNTH-DECLINED-LEG", "h_apv_tm": "123456", "h_stl_amt": "1200"}]},
...
def cases() -> list[Case]:
    ...
    merge = {**reservation_form(job="1202", stnd="Y"), "txtMidRsStnCd": "9099", "txtMidStnConsOrdr": "004", "txtMidStnRunOrdr": "003"}
    ...
    nraw = {**BASE, "lumpStlTgtNo": "SYNTH-LUMP", "dcntCrdStlTgtNo": "SYNTH-N-TARGET", "dcntCrdKndMgNo": "SYNTH-SERVER-KIND",
            "rcvdAmt": "1200", "stxAmt": "100", "taxtSplAmt": "1100", "usePsbTno": "10", "vlidTrmStDt": "20991230", "vlidTrmClsDt": "20991231"}
    return [
        Case("reserve", lambda c: c.reserve(train()), "POST", (reserve,), reservation_form(), hold_raw(), "parse_reservation_hold_response", {"pnr_no": PNR, "received_amount": "1200", "total_price": "9900"}),
        Case("reserve_transfer", lambda c: c.reserve_transfer([train(), train(True)], seat_classes=(Cabin.GENERAL, Cabin.SPECIAL)), "POST", (reserve,), reservation_form(transfer=True, cabin2="2"), hold_raw(), "parse_reservation_hold_response", {"pnr_no": PNR}),
        Case("reserve_merge", lambda c: c.reserve_merge(train(), merge_rows()), "POST", (reserve,), merge, hold_raw(), "parse_reservation_hold_response", {"received_amount": "1200"}),
        Case("reserve_with_discount_card", lambda c: c.reserve_with_discount_card(train(), card_no="SYNTH-N-CARD"), "POST", (reserve,), nreserve, hold_raw(), "parse_reservation_hold_response", {"pnr_no": PNR}),
        Case("confirm_standby_hold", lambda c: c.confirm_standby_hold(hold(), allow_seat_class_change=True, sms_notify=True, phone_no="00000000000"), "POST", ("reservationWait.ReservationWait",), {**COMMON, "txtPnrNo": PNR, "txtPsrmClChgFlg": "Y", "txtSmsSndFlg": "Y", "txtCpNo": "00000000000"}, BASE, None, {}),
        Case("cancel_unpaid_hold", lambda c: c.cancel_unpaid_hold(hold()), "POST", ("reservationCancel.ReservationCancel", "reservationCancel.ReservationCancelChk"), cancel, BASE, None, {}),
        Case("cancel_product_reservation", lambda c: c.cancel_product_reservation(ProductDetailResponse(virtual_reservation_no="SYNTH-PRODUCT", goods_sequence="SYNTH-GOODS")), "GET", ("product.ReservationCancel",), {**COMMON, "txtVrRsNo": "SYNTH-PRODUCT", "txtGdSqno": "SYNTH-GOODS"}, {**BASE, "intgMsgCd": "SYNTH-INTEGRATED"}, "parse_product_cancel_response", {"integrated_message_code": "SYNTH-INTEGRATED"}),
        Case("pay_with_card", lambda c: c.pay_with_card(hold(), card()), "POST", ("payment.ReservationPayment",), payment_form(), payment_raw(), "parse_reservation_payment_response", {"total_received_amount": "1200"}),
        Case("refund", lambda c: c.refund(ticket()), "POST", ("refunds.RefundsRequest",), refund_form(), {**BASE, "stlList": [{"stl_mns_cd": "02"}, {"stl_mns_cd": "SYNTH-SECOND"}]}, "parse_refund_ticket_response", {"settlement_method_codes": ("02", "SYNTH-SECOND"), "settlement_list_is_null": False}),
        Case("verify_station_ticket_refund", lambda c: c.verify_station_ticket_refund(verification_request()), "POST", ("refunds.verifyOnlineRefunds",), {**COMMON, "strName": "SYNTH-CUSTOMER", "retNo1": "SYNTH-A", "retNo2": "SYNTH-B", "retNo3": "SYNTH-C", "retNo4": "SYNTH-D"}, verification_raw(), "parse_station_refund_verification_response", {"refund_amount": "1100", "refund_fee": "100"}),
        Case("execute_station_ticket_refund", lambda c: c.execute_station_ticket_refund(execution_request()), "POST", ("refunds.executeOnlineRefunds",), execution_form(), {**BASE, "h_ret_dv_cd": "SYNTH-DIV"}, "parse_station_refund_execution_response", {"refund_division_code": "SYNTH-DIV"}),
        Case("add_to_cart", lambda c: c.add_to_cart(CartAddRequest(PNR)), "POST", ("cart.addCartList",), {**COMMON, "hidPnrNo": PNR}, {**BASE, "psgDiscAdd_infos": {"psgDiscAdd_info": [{"h_psg_sqno": "01", "h_duty_ref_rcgn_ps_dv_cd": "SYNTH-DISC"}]}}, "parse_cart_add_response", {}),
        Case("register_discount_card", lambda c: c.register_discount_card(purchase_request()), "POST", ("research.dcntCrdInfo.do",), purchase_form(), nraw, "parse_discount_card_purchase_response", {"lump_settlement_target_no": "SYNTH-LUMP", "registered_card_kind_management_no": "SYNTH-SERVER-KIND", "discount_card_settlement_target_no": "SYNTH-N-TARGET", "received_amount": "1200", "stx_amount": "100", "taxt_supply_amount": "1100", "usable_trip_count": "10", "validity_start_date": "20991230", "validity_end_date": "20991231"}),
        Case("extend_discount_card", lambda c: c.extend_discount_card(DiscountCardTicket("SYNTH-ORIGINAL-WINDOW", "20991230", "SYNTH-ORIGINAL-SEQ", "SYNTH-RETURN-PWD")), "POST", ("reservation.dcntCrdExtn.do",), {**COMMON, "saleWctNo": "SYNTH-ORIGINAL-WINDOW", "saleDd": "20991230", "saleSqno": "SYNTH-ORIGINAL-SEQ", "tkRetPwd": "SYNTH-RETURN-PWD"}, BASE, None, {}),
        Case("recalculate_price", lambda c: c.recalculate_price(recalc_request()), "POST", ("certification.PriceReCalculation",), recalc_form(), hold_raw(), "parse_reservation_hold_response", {"pnr_no": PNR, "received_amount": "1200"}),
    ]
...
class Harness:
    def __init__(self, responses: list[Any], method: str, routes: tuple[str, ...], expected: dict[str, Any]) -> None:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            return httpx.Response(200, content=json.dumps(responses[i]).encode("utf-8"), headers={"Content-Type": "application/json"})
        cfg = KorailConfig(base_url="https://offline.invalid", device=COMMON["Device"], version=COMMON["Version"], key=COMMON["Key"], lang=COMMON["lang"],
                           netfunnel_enabled=False, dynapath=DynapathConfig(enabled=True, token_provider=lambda ctx: "SYNTHETIC-OFFLINE-NOT-A-TOKEN"))
...
def test_each_public_method_entire_wire_form_and_parse(case: Case) -> None:
    ...
    try:
        ...
        if case.name == "pay_with_card":
            assert len(result.tickets) == len(result.settlements) == len(result.coupons) == len(result.table_seats) == 1
            assert result.tickets[0].raw == case.response["tk_infos"]["tk_info"][0]
            assert result.settlements[0].raw["h_stl_rlt"] == "SYNTH-DECLINED-LEG"  # Do not collapse partial results into success.
...
def test_parser_failure_retains_whole_response_and_never_retries(case: Case, kind: str, monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.parametrize("first_result,stops", [("FAIL", True), ("SUCC", False), ("SYNTH-UNKNOWN", False), (None, False), ("", False)])
def test_cancel_two_stage_non_fail_policy(first_result: Any, stops: bool) -> None:
    ...
    responses = [{**BASE, "strResult": first_result, "h_msg_cd": "SYNTH-FIRST"}, {**BASE, "h_msg_cd": "SYNTH-FINAL"}]
...
def test_merge_exact_four_overrides(standing: bool) -> None:
    ...
    merged = {**first, "txtStndFlg": "Y" if standing else "N", "txtMidRsStnCd": "9099", "txtMidStnConsOrdr": "004", "txtMidStnRunOrdr": "003"}
    changed = {key for key in first.keys() | merged.keys() if first.get(key) != merged.get(key)}
    assert changed == ({"txtStndFlg"} if standing else set()) | {"txtMidRsStnCd", "txtMidStnConsOrdr", "txtMidStnRunOrdr"}
...
def test_refund_two_screen_forms(with_commission: bool) -> None:
    ...
    if with_commission:
        kwargs["commission"] = RefundCommissionResponse(str_result="SUCC", ticket_return_times_division_code="SYNTH-TIMES")
...
@pytest.mark.parametrize("changes", [
    {"card_password": ""}, {"card_password": "0"}, {"card_password": "000"},
    {"birthday": "00000"}, {"birthday": "0000000"}, {"card_type": "S", "birthday": "000000"},
    {"card_expire": "9900"}, {"card_expire": "9913"}, {"card_expire": "2608"}, {"card_expire": "202609"}, {"card_expire": "AB12"},
    {"card_number": ""}, {"card_number": "0000-0000"},
])
...
def test_card_current_month_allowed_and_auth_type(type_: str, auth: str) -> None:
    ...
    h = Harness([BASE], "POST", ("payment.ReservationPayment",), payment_form(hidAthnDvCd1=type_, hidAthnVal1=auth, hidCrdVlidTrm1="2609"))
    try:
...
@pytest.mark.parametrize("changes", [{"pnr_no": ""}, {"window_no": None}, {"received_amount": None}, {"received_amount": "-1"}, {"str_result": "FAIL"}])
...
def test_real_hold_parse_failure_keeps_committed_identifiers(damage: str) -> None:
    ...
    raw = hold_raw()
    if damage == "mismatch": raw["h_tot_rcvd_amt"] = "1201"
    elif damage == "journey-shape": raw["jrny_infos"] = []
    elif damage == "seat-shape": raw["jrny_infos"]["jrny_info"][0]["seat_infos"] = []
    else: raw["h_pnr_no"] = {"synthetic": "bad shape"}
...
@pytest.mark.parametrize("variant,expected", [("padded", "1200"), ("no-seats", "1200"), ("incomplete-seat", None), ("blank-zero", "1200")])
def test_received_amount_never_becomes_partial_sum(variant: str, expected: str | None) -> None:
    raw = hold_raw()
    if variant == "no-seats": raw.pop("jrny_infos")
    elif variant == "incomplete-seat": raw["jrny_infos"]["jrny_info"][0]["seat_infos"]["seat_info"][1].pop("h_rcvd_amt")
    elif variant == "blank-zero": raw["jrny_infos"]["jrny_info"][0]["seat_infos"]["seat_info"].append({"h_seat_no": "", "h_rcvd_amt": "0"})
    h = Harness([raw], "POST", ("certification.TicketReservation",), reservation_form())
    try:
...
def test_station_verification_no_commonout_envelope_and_conversion() -> None:
    ...
    try:
        ...
        request = StationRefundExecutionRequest.from_verification(verified, customer_phone="00000000000", customer_name="SYNTH-CUSTOMER")
...
def test_all_passenger_rows_and_combination_rules() -> None:
    ...
    for i, (type_, discount) in enumerate([("1", "000"), ("1", "P11"), ("3", "000"), ("3", "321"), ("1", "131"), ("1", "111"), ("1", "112"), ("1", "173")], 1):
...
@pytest.mark.parametrize("passengers", [KorailPassengerCounts(adult=0, infant=1), KorailPassengerCounts(adult=0, child=1, infant=1), KorailPassengerCounts(adult=1, guide_dog=1)])
def test_invalid_passenger_combinations_before_http(passengers: KorailPassengerCounts) -> None:
    ...
    h = Harness([], "POST", (), {})
    try:
        with pytest.raises(KorailProtocolError): h.client.reserve(train(), passengers=passengers)
        assert not h.seen
    finally:
        h.close()
...
def test_transfer_seat_and_attribute_indexing() -> None:
    ...
    expected.update(txtSeatAttCd4="021", txtSeatAttCd4_1="028", txtSrcarCnt="1", txtSrcarNo1="1", txtSeatNo1="1A",
                    txtSrcarCnt1="1", txtSrcarNo1_1="2", txtSeatNo1_1="2B")
    h = Harness([hold_raw()], "POST", ("certification.TicketReservation",), expected)
    try:
        h.client.reserve_transfer([train(), train(True)], job_type=Job.SEAT_DESIGNATED,
                                 seats=[(KorailSeatAssignment(1, "1A"),), (KorailSeatAssignment(2, "2B"),)], seat_attribute_codes=["021", "028"])
    finally:
        h.close()
...
def test_recalculation_parallel_empty_slots_and_nonmember_options() -> None:
    ...
    expected = recalc_form(hiduserYn="N", hidCustNo="SYNTH-NONMEMBER", txtPsrmClCd1="2", txtSeatAttCd2="000", txtSeatAttCd4="015", txtSeatAttCd5="000")
    h = Harness([hold_raw()], "POST", ("certification.PriceReCalculation",), expected)
    try:
        h.client.recalculate_price(recalc_request(non_member_no="SYNTH-NONMEMBER", cabin_class_code="2", seat_attribute_code_2="000", seat_attribute_code_4="015", seat_attribute_code_5="000"))
    finally:
        h.close()
...
def test_unsupported_maas_has_no_incoming_import() -> None:
    ...
    for source in root.glob("*.py"):
        if source.name == "_maas_unsupported.py": continue
        for node in ast.walk(ast.parse(source.read_text())):
            ...
            if any("_maas_unsupported" in target for target in targets): incoming.append((source.name, node.lineno))
    assert incoming == []
...
PAYMENT_SCALARS = {
    "reservation_no": "h_rsv_no", "settlement_approval_no": "h_stl_cd_apprv_no", "total_received_amount": "h_tot_rcvd_amt",
    "settlement_amount": "h_stl_amt", "total_settlement_amount": "h_tot_stl_amt", "customer_no": "h_cust_no",
    "member_card_no": "h_mb_crd_no", "buyer_name": "h_buy_name", "publication_start_no": "h_publ_start_no", "publication_end_no": "h_publ_end_no",
    "mixed_settlement_division": "h_mix_stl_dv", "cancellation_fee": "h_cnc_fee", "window_no": "h_wct_no", "settlement_count": "h_stl_cnt",
    "discount_card_count": "h_disc_card_cnt", "total_price": "h_tot_prc", "total_fare": "h_tot_fare", "total_discount_amount": "h_tot_disc_amt",
    "adult_count": "h_adult_cnt", "child_count": "h_child_cnt", "table_seat_count": "h_tbl_seat_cnt", "ticket_count": "h_tk_cnt",
    "settlement_type_code": "h_stl_tp_cd", "trade_division": "h_trade_gbn", "remark": "h_bigo", "survey_flag": "h_survey_flg",
    "survey_title": "h_survey_title", "survey_text": "h_survey_text", "survey_url": "h_survey_url", "image_ticket_flag": "h_im_flg",
}
PAYMENT_ROWS = {
    "tickets": ("tk_infos", "tk_info", {
        "ticket_sequence": "h_tk_sqno", "sale_date": "h_sale_dt", "sale_sequence": "h_sale_sqno", "return_password": "h_tk_ret_pwd", "return_no": "h_tk_ret_no",
        "recipient_name": "h_take_name", "discount_card_no": "h_disc_card_no", "ticket_price": "h_tk_prc", "ticket_fare": "h_tk_fare",
        "bz5_fare_discount_amount": "h_bz5_fare_disc_amt", "bz6_fare_discount_amount": "h_bz6_fare_disc_amt", "total_discount_amount": "h_tot_disc_amt",
        "total_received_amount": "h_tot_rcvd_amt", "standard_seat_price_fare": "h_std_seat_prc_fare"}),
    "settlements": ("stl_infos", "stl_info", {
        "settlement_sequence": "h_stl_sqno", "settlement_type_code": "h_stl_tp_cd", "settlement_result": "h_stl_rlt", "transaction_division": "h_tr_gubun",
        "card_installment_count": "h_crd_stl_cnt", "installment_months": "h_inst_month", "settlement_amount": "h_stl_amt", "settlement_card_no": "h_stl_crd_no",
        "card_company_code": "h_crd_corp_cd", "card_company_name": "h_crd_corp_nm", "approval_date": "h_apv_dt", "approval_time": "h_apv_tm",
        "approval_no": "h_apv_no", "point_division": "h_xpoint_dv", "point_no": "h_xpoint_no", "point_approval_no": "h_xpoint_apv_no",
        "remnant_amount": "h_remnant_amt", "remote_point": "h_rmt_point"}),
    "table_seats": ("tbl_seat_infos", "tbl_seat_info", {
        "room_class_name_1": "h_psrm_cl_cd_nm1", "car_no_1": "h_srcar_no1", "seat_no_start_1": "h_tbl_seat_no_sno_1", "seat_no_end_1": "h_tbl_seat_no_eno_1",
        "seat_count_1": "h_tbl_seat_cnt_1", "group_name_1": "h_sgr_nm_1", "room_class_name_2": "h_psrm_cl_cd_nm2", "car_no_2": "h_srcar_no2",
        "seat_no_start_2": "h_tbl_seat_no_sno_2", "seat_no_end_2": "h_tbl_seat_no_eno_2", "seat_count_2": "h_tbl_seat_cnt_2", "group_name_2": "h_sgr_nm_2"}),
    "coupons": ("tk_coupon_info", None, {"certificate_password": "h_cert_pwd", "coupon_no": "h_coup_no", "management_close_date": "h_fdcert_mg_cls_dt",
        "management_start_date": "h_fdcert_mg_st_dt", "ticket_return_no": "h_tk_ret_no"}),
}
...
def test_all_payment_model_fields_and_optional_masks(mode: str) -> None:
    ...
    def value(wire: str) -> Any:
        return ("SYNTH-" + wire) if mode == "strings" else 123 if mode == "integers" else {"synthetic": "malformed optional"}
...
def test_standby_initial_form_is_not_standing_reservation() -> None:
    ...
    h = Harness([hold_raw()], "POST", ("certification.TicketReservation",), expected)
    try:
        h.client.reserve(train(general_reservation_code="13", standing_reservation_code="11"), job_type=Job.STANDBY)
    finally:
        h.close()


@pytest.mark.parametrize("case", [c for c in CASES if c.name in {"reserve", "reserve_transfer", "reserve_with_discount_card", "pay_with_card"}], ids=lambda c: c.name)
...
def test_numeric_zero_identifiers_are_echoed_without_repadding() -> None:
    ...
    try:
        h.client.pay_with_card(hold(temporary_job_sequence_1="0", temporary_job_sequence_2="42",
                                  journeys=(ReservationJourney(journey_sequence="1", reservation_change_no="0"),)), card())
    finally:
        h.close()
...
def test_invalid_json_retains_original_body_without_retry() -> None:
    ...
    config = KorailConfig(base_url="https://offline.invalid", netfunnel_enabled=False,
...
@pytest.mark.parametrize("shape", ["missing", {}, "bad-list", [None], [1], [[]], [{}], [{"stl_mns_cd": None}], [{"stl_mns_cd": []}], [{"stl_mns_cd": {}}], [{"stl_mns_cd": True}]])
def test_refund_required_settlement_mask_does_not_silently_drop_data(shape: Any) -> None:
    ...
    raw = copy.deepcopy(BASE)
    if shape != "missing": raw["stlList"] = shape
...
@pytest.mark.parametrize("rows,codes,is_null", [(None, (), True), ([], (), False), ([{"stl_mns_cd": ""}], ("",), False), ([{"stl_mns_cd": 2}], ("2",), False)])
def test_refund_required_but_nullable_list_and_integer_string_compatibility(rows: Any, codes: tuple[str, ...], is_null: bool) -> None:
...
def test_actual_first_and_merge_requests_differ_only_at_four_fields() -> None:
    ...
    try:
        ...
        changed = {k for k in actual_first.keys() | actual_second.keys() if actual_first.get(k) != actual_second.get(k)}
...
def test_hold_response_is_actual_source_of_payment_identifiers_and_amount() -> None:
    ...
    h = Harness([hold_raw(), payment_raw()], "POST", ("certification.TicketReservation", "payment.ReservationPayment"), reservation_form())
    try:
        response = h.client.reserve(train())
