...
def test_repository_truth_and_full_mutation_policy() -> None:
    ...
    for document in (readme, handoff):
        ...
        # 78, not 72 or 74. The superseded numbers still appear in the handoff,
        # because those sentences were true when written and are kept as
        # history; the pin has to name the CURRENT boundary or it stops
        # detecting the next drift.
        assert "78 public methods" in document
        assert "2264 passed" in document and "1 deselected" in document
